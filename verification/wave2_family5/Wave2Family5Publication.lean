...
namespace Wave2Family5Publication

/-! Finite scoped definitions. -/
...
/-! Structural list helpers for the core-only pigeonhole proof. -/
...
theorem mem_map_witness
  ...
  | cons a xs ih =>
      ...
      | head =>
          exact ⟨a, .head _, rfl⟩
      | tail _ hTail =>
          obtain ⟨a', ha', hfa'⟩ := ih hTail
          exact ⟨a', .tail _ ha', hfa'⟩
...
/-! General finite non-injective collision theorem. -/
...
theorem finiteList_noninjective_collision
  ...
  | cons s tail ih =>
      ...
      · have hHeadRange : encode s ∈ codes :=
        ...
        exact
          ⟨s₁, .tail _ hs₁, s₂, .tail _ hs₂, hNe, hCode⟩

/-! Exact fiber-factorization theorem. -/
...
/-! Equivalence-relation generalization. -/

structure RelEquivalence
    {A : Type u}
    (rel : A → A → Prop) : Prop where
  refl : ∀ a : A, rel a a
...
/-! Exact finite enumerations. -/

structure ExactEnum (A : Type u) where
  elems : List A
  nodup : elems.Nodup
  complete : ∀ a : A, a ∈ elems
...
/-! Decision obstruction and stateful-provider carrier extension. -/
...
def frozenObserved
    ...
    (observe : O × St → A)
    (input : S) : A :=
  observe (frozenTransition transition frozenState input)
...
/-! Installed decoder versus the actually deployed fixed-model path. -/
...
def boolNotEncoding (b : Bool) : Bool :=
  !b

def boolNotDecoder (b : Bool) : Bool :=
  !b

def boolIdentity (b : Bool) : Bool :=
  b
...
theorem boolNot_fiberCompatible :
    FiberCompatibleOn
      ...
      boolNotEncoding
      boolIdentity := by
  intro b₁ _hb₁ b₂ _hb₂ hCode
  cases b₁ <;> cases b₂ <;> cases hCode <;> rfl
...
theorem decoderInputPathBridge_implies_fixedModelPreserves
    {S : Type u} {C : Type v} {I : Type w} {O : Type x}
    (states : List S)
    (encode : S → C)
    (decode : C → S)
    (rawInput : S → I)
