...
- **Bypass matrix:** `raw=true` (edit-pass contract requires exact bytes), passthrough mode, per-call `fresh: true`, env `TOKENZERO_MCP_DEDUP=0|off` (default on). `expand` is never deduped. Classic MCP/CLI expand remains exact; CodeMode bounds large defaults and requires `{raw:true}` or an explicit line/symbol window for exact recovery.
