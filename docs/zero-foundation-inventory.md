...
| QuickJS CodeMode host, sandbox, and plan wrapping | TZ `crates/tokenzero-codemode/src/exec.rs`, `crates/tokenzero-codemode/src/sandbox.rs`, `crates/tokenzero-codemode/src/parser.rs`, and `crates/tokenzero-codemode/src/result.rs`; FS `src/codemode/host.rs`, `src/codemode/js.rs`, and `src/codemode/plan.rs`; GZ `crates/graphzero-query/src/codemode/quickjs.rs`, `crates/graphzero-query/src/codemode/plan.rs`, and `crates/graphzero-query/src/codemode/response.rs`; hub `crates/zero-codemode/src/host.rs`, `crates/zero-codemode/src/wrap.rs`, and `crates/zero-codemode/src/limits.rs`; exact tests below | `CodeModeHost`, `SandboxLimits`, `Plan`, `PlanStep`, `WrappedResult`, and `execute(plan, limits)`. Depend on `ZeroStack::zero-codemode` for host/sandbox/wrapping contracts and `ZeroStack::zero-abi` for wire envelopes. Engines retain domain operations and policy callbacks. | TokenZero **220--420**; FSZero **300--600**; GraphZero **300--650** |
...
| Error/result envelopes | TZ `crates/tokenzero-codemode/src/result.rs` and `crates/tokenzero-engine/src/codemode_wire.rs`; FS `src/core/op_result.rs` and `src/codemode/zero_result.rs`; GZ `crates/graphzero-query/src/codemode/errors.rs`, `crates/graphzero-query/src/codemode/response.rs`, and `crates/graphzero-query/src/codemode/types.rs`; hub `crates/zero-abi/src/result.rs`; exact tests below | `ResultEnvelope<T>`, `ErrorEnvelope`, `ErrorCode`, `Ack2`, and stable serialization/version checks. Depend on `ZeroStack::zero-abi`; engines retain domain error construction and recovery policy. | TokenZero **100--220**; FSZero **80--180**; GraphZero **80--180** |
...
- **TokenZero:** `crates/tokenzero-codemode/src/exec.rs` defines `execute_codemode`, `execute_codemode_with_options`, `execute_quickjs_plan`, and `wrap_js_plan`; `sandbox.rs` defines `lower_code_plan`; `parser.rs` defines `parse_plan`; `result.rs` defines `CodeModeOptions`, `CodeModeResult`, `CodeModeStatus`, and `CodeModeError`; `crates/tokenzero-engine/src/codemode_wire.rs` defines `CodeModeLimits` and `codemode_execute`. Exact tests are `crates/tokenzero-codemode/src/containment_tests.rs`, `crates/tokenzero-codemode/src/e2e_tests.rs`, and `crates/tokenzero-engine/tests/codemode_bindings_dispatcher.rs`.
...
- **TokenZero:** `crates/tokenzero-codemode/src/result.rs` defines `CodeModeResult`, `CodeModeError`, `CodeModeTelemetry`, `CodeModeStatus`, `completed`, and `error`; `crates/tokenzero-engine/src/codemode_wire.rs` defines `CodeModeResult`, `CodeModeError`, `CodeModeTelemetry`, `classify_method`, and `classify_descriptor_tool`. Exact current tests are `crates/tokenzero-engine/tests/codemode_bindings_dispatcher.rs`, `crates/tokenzero-mcp-compat/tests/jsonrpc_conformance.rs`, and the fixture `crates/tokenzero-core/tests/fixtures/ack2-golden.json`.
...
check_paths . HEAD \
  ...
  crates/tokenzero-codemode/src/exec.rs \
  crates/tokenzero-codemode/src/sandbox.rs \
  crates/tokenzero-codemode/src/parser.rs \
  crates/tokenzero-codemode/src/result.rs \
  crates/tokenzero-engine/src/codemode_wire.rs \
  crates/tokenzero-codemode/src/containment_tests.rs \
  crates/tokenzero-codemode/src/e2e_tests.rs \
