...
$RepoDir = Resolve-Path (Join-Path $DemoDir '..') | Select-Object -ExpandProperty Path
$BinDir   = Join-Path $DemoDir '.tokenzero-bin'
$CacheDir = Join-Path $DemoDir '.cache'
$null = New-Item -ItemType Directory -Force -Path $BinDir, $CacheDir

$CachePath   = Join-Path $CacheDir 'recovery-cache.json'
$CountCache  = Join-Path $CacheDir 'count-cache.json'
$ResultsPath = Join-Path $DemoDir 'demo_results.json'
...
function Resolve-TokenZeroBinary {
    ...
    $expected = ((Get-Content -LiteralPath $sha -Raw).Trim() -split '\s+')[0].ToLower()
...
Write-Host "==> Using binary: $Tz"
$tzVersion = (& $Tz --version) -join ' '
Write-Host "    $tzVersion"
...
Write-Host ''
Write-Host '=== TokenZero demo ===' -ForegroundColor Green
Write-Host "Repo: $RepoDir"
Write-Host "Cache (isolated): $CachePath"
Write-Host ''
...
if (Test-Path -LiteralPath $largeFile) {
    ...
    $resObj   = ConvertFrom-Json $resJson
    $visTok   = $resObj.accounting.visible_tokens
    $largeRef = ($resObj.refs | Where-Object { $_.kind -eq 'blob' } | Select-Object -First 1).ref
    Add-Row "large read ($largeFile)" $rawTok $visTok "ref: $largeRef"
}
...
if (Test-Path -LiteralPath $largeFile) {
    ...
    if (-not $secondCallText) {
        ...
        $rawTok = Get-RawTokens (Get-Content -LiteralPath $largeFile -Raw)
        $visTok = Get-RawTokens $secondCallText
        Add-Row "re-read same file (MCP dedup)" $rawTok $visTok 'second call routed through seen-set in same MCP session'
    }
}
...
if ($largeRef) {
    ...
    $rec = $recovered.TrimEnd("`r","`n")
    $orig = $original.TrimEnd("`r","`n")
    if ($rec -ne $orig) {
...
try {
    ...
    $resJson = Invoke-Tz -ArgList (@('run','--json','--cache-path', $CachePath, '--') + $probeCmd)
    $visTok  = Get-VisibleTokens $resJson
...
# --- Summary ----------------------------------------------------------------
$totalRaw     = ($rows | Where-Object { $_.raw_tokens     -gt 0 } | Measure-Object -Sum raw_tokens).Sum
$totalVisible = ($rows | Where-Object { $_.raw_tokens     -gt 0 } | Measure-Object -Sum visible_tokens).Sum
...
$nameW = 40
$head = ('{0,-' + $nameW + '} {1,12} {2,12} {3,10}  {4}') -f 'Workload','Raw tokens','Visible','Savings','Note'
Write-Host $head
Write-Host ('-' * ($head.Length))
foreach ($r in $rows) {
...
Write-Host ('-' * ($head.Length))
$totalLine = ('{0,-' + $nameW + '} {1,12} {2,12} {3,10}') -f 'TOTAL (rows with raw baseline)', $totalRaw, $totalVisible, (Format-Pct $totalRaw $totalVisible)
Write-Host $totalLine -ForegroundColor Cyan
Write-Host ''
Write-Host 'Honest accounting: every TokenZero row above keeps exact tz:// refs.'
Write-Host 'Hidden bytes are one `tokenzero expand <ref>` call away — and scenario 5'
Write-Host 'proves the round-trip really is byte-exact.'
Write-Host ''

$payload = [pscustomobject]@{
    tokenzero_version = $tzVersion
    binary            = $Tz
    repo              = $RepoDir
    cache             = $CachePath
    workloads         = $rows
    totals            = [pscustomobject]@{
        raw_tokens     = $totalRaw
        visible_tokens = $totalVisible
        savings_pct    = $totalPct
    }
}
$payload | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath $ResultsPath -Encoding UTF8
...
if (-not $NoViz) {
    ...
    if (Test-Path -LiteralPath $vizScript) {
        Write-Host ''
        Write-Host '==> Rendering demo_viz.html' -ForegroundColor Cyan
