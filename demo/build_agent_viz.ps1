...
.DESCRIPTION
    Reads demo\agent_results.json on a timer (in the browser) and renders; a live table + summary cards. Single static HTML page; the only JS is; a tiny fetch+render loop. No CDN, no frameworks.
#>
[CmdletBinding()]
param(
    [string] $OutPath,
    ...
    [int]    $Port = 8765,
    [switch] $Open
); $ErrorActionPreference = 'Stop'; $DemoDir = Split-Path -Parent $MyInvocation.MyCommand.Path
if (-not $OutPath) { $OutPath = Join-Path $DemoDir 'agent_viz.html' }

$html = @"; <!doctype html>; <html lang="en">; <head>; <meta charset="utf-8">; <meta name="viewport" content="width=device-width,initial-scale=1">; <title>TokenZero &mdash; live agent runs</title>; <style>
:root {
...
<header class="page">
  <h1><span class="live-dot" id="dot"></span>TokenZero &mdash; live agent runs</h1>; <p class="sub">Real Copilot CLI sessions on the JSON-RPC-errors task &middot;
     baseline vs TokenZero-only &middot; updates every $RefreshMs ms &middot;; data: <code id="data-path">$DataPath</code> &middot;; <span id="status">connecting&hellip;</span></p>
</header>

<section class="cards" id="cards"></section>; <section class="split" id="split"></section>
...
<section class="panel" style="background:var(--bg-elev); border:1px solid var(--border); border-radius:10px; padding:0; overflow:hidden;">
  <table id="runs">
    <thead>
      <tr>
        <th>#</th><th>Condition</th><th>Status</th><th>Wall</th><th>Input tok</th>; <th>Output tok</th><th>Tool calls</th><th>Tool-out tok</th>; <th>API ms</th><th>Notes</th>
      </tr>
    </thead>; <tbody id="rows"><tr><td colspan="10" class="empty">waiting for runs&hellip;</td></tr></tbody>
  </table>
</section>

<footer class="page">
  Generated by <code>demo\build_agent_viz.ps1</code>. Re-render any time with; <code>pwsh -File demo\build_agent_viz.ps1 -Open</code>.; Combined report (perf + bugs + live runs) at <a href="demo_viz.html">demo_viz.html</a>.
</footer>

</div>; "@ + @'; <script>; const DATA_URL = "__DATA_URL__";; const REFRESH_MS = __REFRESH_MS__;; const fmt = n => (n == null || n === '') ? '\u2014'
...
async function tick() {
  try {
    ...
    const j = await res.json();; render(j);; byId('status').textContent = 'live (updated ' + new Date().toLocaleTimeString() + ')';; byId('status').className = '';; byId('dot').style.background = 'var(--accent)';
  } catch (e) {
    byId('status').textContent = 'no data yet (' + e.message + ')';; byId('status').className = 'disconnected';; byId('dot').style.background = 'var(--fg-dim)';
  }
}

function render(j) {
  const runs = j.runs || [];; const meta = j.meta || {};; const totals = j.totals || {};
  ...
  const cards = [
    ...
    '<div class="card"><div class="lbl">' + esc(c[0]) + '</div>' +; '<div class="num">' + esc(c[1]) + '</div>' +; (c[2] ? '<div class="sub">' + esc(c[2]) + '</div>' : '') + '</div>'
  ).join('');

  const summary = j.summary || {baseline:{}, tokenzero:{}};
  const splitHtml = ['baseline','tokenzero'].map(k => {
    const s = summary[k] || {};; const meanTok = s.mean_tool_output_tokens || 0;
    const maxTok = Math.max(
      (summary.baseline||{}).mean_tool_output_tokens || 0,
      (summary.tokenzero||{}).mean_tool_output_tokens || 0, 1);
    const pct = Math.round((meanTok / maxTok) * 100);; const title = k === 'baseline' ? 'Run A \u2014 native tools' : 'Run B \u2014 TokenZero MCP only';; const sd = s.stddev_tool_output_tokens != null ? ', \u03c3=' + Math.round(s.stddev_tool_output_tokens) : '';
    const subN = s.n ? ' (n=' + s.n + sd + ')' : '';; const mc = (s.mean_tool_calls != null) ? Number(s.mean_tool_calls).toFixed(1) : '\u2014';
    return '<div class="panel ' + k + '"><h3>' + title + '</h3>' +
      '<div class="big">' + fmt(Math.round(meanTok)) + '</div>' +; '<div class="meta">mean tool-output tokens per run' + subN + '</div>' +; '<div class="bar-mini"><span style="width:' + pct + '%"></span></div>' +; '<div class="meta" style="margin-top:10px">' +
        'tool calls: ' + mc +; ' \u00b7 wall: ' + fmtTime(s.mean_wall_ms) +; ' \u00b7 in tok: ' + fmt(Math.round(s.mean_input_tokens || 0)) +; ' \u00b7 out tok: ' + fmt(Math.round(s.mean_output_tokens || 0)) +
      '</div></div>';
  }).join('');; byId('split').innerHTML = splitHtml;
  ...
  byId('rows').innerHTML = runs.map(r => {
    const cls = css(r.status || 'pending');; const condition = css(r.condition || '');; const statusBadge = r.status === 'running'
      ? '<span class="spinner"></span><span class="pill running">running</span>'; : '<span class="pill ' + cls + '">' + esc(r.status || 'pending') + '</span>';
    return '<tr class="' + cls + '">' +
      '<td>' + esc(r.index != null ? r.index : '') + '</td>' +; '<td><span class="pill ' + condition + '">' + esc(r.condition || '') + '</span></td>' +; '<td>' + statusBadge + '</td>' +; '<td>' + esc(fmtTime(r.wall_ms)) + '</td>' +; '<td>' + esc(fmt(r.input_tokens)) + '</td>' +
      '<td>' + esc(fmt(r.output_tokens)) + '</td>' +; '<td>' + esc(fmt(r.tool_calls)) + '</td>' +; '<td>' + esc(fmt(r.tool_output_tokens)) + '</td>' +; '<td>' + esc(fmt(r.api_ms)) + '</td>' +; '<td class="muted">' + esc(r.note || '') + '</td>' +
    '</tr>';
  }).join('');
}

tick();; setInterval(tick, REFRESH_MS);; </script>; </body>; </html>; '@
...
if ($Open) {
    ...
    if ($py) {
        $null = Start-Process -FilePath $py.Source -ArgumentList @('-m','http.server',"$Port",'--bind','127.0.0.1') -WorkingDirectory $DemoDir -PassThru; Start-Sleep -Milliseconds 400; Start-Process ("http://127.0.0.1:$Port/" + [System.IO.Path]::GetFileName($OutPath))
    } else {
