...
fn canonical_cli_has_no_raw_worker_entrypoint_or_default_adapter() {
    ...
    assert!(cargo.contains("name = \"tokenzero-mcp\""));
    assert!(cargo.contains("required-features = [\"surface-mcp\"]"));
...
fn installer_is_selector_probe_uninstall_only_until_central_discovery() {
    ...
    assert!(installer.contains("classic MCP compatibility is built separately with surface-mcp"));
