use crate::catalog::{
    TOOL_ALIASES, ToolKind, canonical_tool_specs, resource_specs, tool_specs, tool_specs_for_filter,
};
use serde_json::{Value, json};
use std::collections::BTreeSet;
use tokenzero_core::McpToolSurface;
use tokenzero_core::operation_abi::{
    CancellationSemantics, CostClass, DomainErrorKind, Mutability, RefOwnership, all_operations,
    contract_digest_hex, golden_vectors, operation_by_name, resolve_operation, resource_uris,
    schema_diff, schemas_structurally_equal,
};
...
fn every_codemode_method_resolves_and_has_io_schemas_on_describe() {
    for path in live_codemode_paths() {
        let op =
            resolve_operation(path).unwrap_or_else(|| panic!("CodeMode path {path} not in ABI"));
