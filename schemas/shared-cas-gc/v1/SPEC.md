...
This document freezes the engine-neutral metadata contract identified by `zerostack.cas-gc.v1`. TokenZero owns the contract; TokenZero, FSZero, and GraphZero are peer producers and consumers. RFC 2119 terms are normative.
...
Every record has `schema_version: "zerostack.cas-gc.v1"` and a record-specific `record_type`. JSON is UTF-8. Producers MUST emit schema-valid records. Consumers MUST reject duplicate object keys and non-finite numbers. v1 consumers MUST treat any other schema version as unsupported, including lexically similar newer versions.
