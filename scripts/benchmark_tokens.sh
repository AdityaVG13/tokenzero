...
TZ_BIN="${1:-$HOME/.tokenzero/bin/tokenzero}"
...
# --- 1. Large file read ---------------------------------------------------
FILE=crates/tokenzero-mcp/src/lib.rs
RAW=$(count_tokens < "$FILE")
VIS=$("$TZ_BIN" read "$FILE" --json --cache-path "$BENCH_CACHE" | visible_of)
...
# --- 3. Repo-wide grep ------------------------------------------------------
RAW=$( (rg -n --no-heading --color=never 'fn ' crates/ || true) | count_tokens)
VIS=$("$TZ_BIN" grep 'fn ' crates --json --max-files 200 --cache-path "$BENCH_CACHE" | visible_of)
row "repo-wide grep ('fn ' across crates/)" "$RAW" "$VIS"; add_totals "$RAW" "$VIS"

# --- 4. Test-suite run ------------------------------------------------------
RAW=$( (command cargo test -p tokenzero-filters 2>&1 || true) | count_tokens)
VIS=$("$TZ_BIN" run --json --cache-path "$BENCH_CACHE" -- cargo test -p tokenzero-filters | visible_of)
row "cargo test run (tokenzero-filters suite)" "$RAW" "$VIS"; add_totals "$RAW" "$VIS"

# --- 5. Directory tree ------------------------------------------------------
RAW=$( (find crates -type f || true) | count_tokens)
VIS=$("$TZ_BIN" tree crates --depth 3 --json --cache-path "$BENCH_CACHE" | visible_of)
row "directory listing (find vs tree, depth 3)" "$RAW" "$VIS"; add_totals "$RAW" "$VIS"

# --- 6. Re-find content: recall vs re-running the search --------------------
RAW=$( (rg -n --no-heading --color=never 'fn ' crates/ || true) | count_tokens)
VIS=$("$TZ_BIN" recall 'fn main' --max-hits 10 --json --cache-path "$BENCH_CACHE" | visible_of)
