...
EXTRACTED_MODULES = [
    "crates/tokenzero-core/src/render/domain.rs",
    "crates/tokenzero-core/src/render/noise.rs",
    "crates/tokenzero-core/src/shell_display.rs",
    "crates/tokenzero-core/src/shell_parse.rs",
    "crates/tokenzero-core/src/tokens.rs",
    "crates/tokenzero-mcp/src/collect.rs",
    "crates/tokenzero-mcp/src/paths.rs",
    "crates/tokenzero-mcp/src/render.rs",
    "crates/tokenzero-install/src/content.rs",
    "crates/tokenzero-install/src/doctor.rs",
    "crates/tokenzero-install/src/inspect.rs",
    "crates/tokenzero-install/src/windows_path.rs",
...
FACADE_CEILINGS = {
    "crates/tokenzero-core/src/lib.rs": 2500,
...
INTERNAL_MODULES = {
    "tokenzero_core": ["shell_parse", "tokens", "render", "shell_display"],
    "tokenzero_mcp": ["collect", "paths", "render"],
    "tokenzero_install": ["doctor", "inspect", "content", "windows_path"],
}
...
def main() -> int:
    ...
    for rel in EXTRACTED_MODULES:
        if not (ROOT / rel).is_file():
            violations.append(f"{rel}: extracted module file is missing "
                              "(was it re-inlined?)")
    ...
    for rel, ceiling in FACADE_CEILINGS.items():
        ...
        if count > ceiling:
            violations.append(f"{rel}: {count} lines exceeds the {ceiling}-line "
                              "facade ceiling; extract a module instead")

    deep = re.compile(
        "|".join(rf"{crate}::(?:{'|'.join(mods)})::"
                 for crate, mods in INTERNAL_MODULES.items())
    )
    for path in sorted(ROOT.glob("crates/*/src/**/*.rs")) + sorted(
        ROOT.glob("crates/*/tests/**/*.rs")
    ) + sorted(ROOT.glob("crates/*/benches/**/*.rs")):
        text = path.read_text()
        for i, line in enumerate(text.splitlines(), 1):
            m = deep.search(line)
            if m:
                violations.append(f"{path.relative_to(ROOT)}:{i}: deep import "
                                  f"`{m.group(0)}...` bypasses the crate facade")
