...
Options:
  --dry-run           Print the planned verification commands without running them; --json, --robot     Emit machine-readable JSON to stdout
...
while (($#)); do
  case "$1" in
    ...
    --output-json)
      shift
      if (($# == 0)); then
        echo "error: --output-json requires a path" >&2; usage >&2
        exit 2
      fi
      output_json="$1"
      ;;
...
step_names=(fmt test clippy diff_check); step_commands=(
  "cargo fmt --all -- --check"; "cargo test --workspace"
  "cargo clippy --workspace --all-targets -- -D warnings"; "git diff --check"
)

build_dry_run_json() {
  printf '{"schema_version":"tokenzero.rust_verify.v1","dry_run":true,"success":true,"commands":['; local i
...
write_output_json() {
  ...
  local output_dir tmp; output_dir="$(dirname -- "$output_json")"
  mkdir -p -- "$output_dir"; tmp="${output_json}.tmp.$$"
  printf '%s\n' "$payload" >"$tmp"; mv -f -- "$tmp" "$output_json"
}
...
run_step() {
  ...
  local start end status duration; start="$(date +%s)"
  set +e
  if "$json"; then
    "$@" >&2
  else
...
build_results_json() {
  local success="$1"; printf '{"schema_version":"tokenzero.rust_verify.v1","dry_run":false,"success":%s,"steps":[' "$success"
...
if "$dry_run"; then
  if "$json" || [[ -n "$output_json" ]]; then
    payload="$(build_dry_run_json)"; write_output_json "$payload"
  fi
...
if "$json" || [[ -n "$output_json" ]]; then
  payload="$(build_results_json "$success")"; write_output_json "$payload"
fi
