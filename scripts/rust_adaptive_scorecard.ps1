...
function Accounting-Row {
  ...
  return [ordered]@{
    ...
    ok = $false
    status = $json.status
    command_success = $telemetry.command_success
    output_strategy = $telemetry.output_strategy
    wall_ms = $Case.wall_ms
    latency_ms = $telemetry.latency_ms
    raw_tokens = $accounting.raw_tokens
    visible_tokens = $accounting.visible_tokens
    recovery_tokens = $accounting.recovery_tokens
    exact_ref_tokens = $accounting.exact_ref_tokens
...
$tinyRow.ok = (
  $tiny.exit_code -eq 0 -and
  $null -eq $tiny.parse_error -and
  $tiny.json.status -eq "ok" -and
  $tiny.json.telemetry.command_success -eq $true -and
  $tiny.json.telemetry.output_strategy -eq "compact_adaptive_shell" -and
  $tiny.json.accounting.visible_tokens -le $tiny.json.accounting.raw_tokens -and
  $tiny.json.telemetry.latency_ms -le 100 -and
  $tiny.wall_ms -le 3500
)
if (!$tinyRow.ok) {
...
$tinyReadDir = Join-Path ([System.IO.Path]::GetTempPath()) ("tokenzero-long-label-" + [guid]::NewGuid().ToString("N"))
$tinyReadFile = Join-Path $tinyReadDir "tiny.md"
try {
  New-Item -ItemType Directory -Force $tinyReadDir | Out-Null
  ...
  $tinyReadRow.ok = (
    ...
    $null -eq $tinyRead.parse_error -and
    $tinyRead.json.status -eq "ok" -and
    $tinyRead.json.visible.text.Contains("ok") -and
    !$tinyRead.json.visible.text.Contains("omitted") -and
    $tinyRead.json.accounting.visible_tokens -le 20
  )
  if (!$tinyReadRow.ok) {
...
$powerShellRouteRow.ok = (
  $powerShellRoute.exit_code -eq 0 -and
  $null -eq $powerShellRoute.parse_error -and
  $powerShellRoute.json.status -eq "ok" -and
  $powerShellRoute.json.telemetry.command_success -eq $true -and
  $powerShellRoute.json.telemetry.execution_mode -eq "shell" -and
  $powerShellRoute.json.telemetry.argv[0] -eq "powershell" -and
  $powerShellRoute.json.telemetry.stdout_preview.EndsWith("tz-quote") -and
  $powerShellRoute.wall_ms -le 3500
)
...
$timeoutRouteRow.ok = (
  $timeoutRoute.exit_code -eq 0 -and
  $null -eq $timeoutRoute.parse_error -and
  $timeoutRoute.json.status -eq "ok" -and
  $timeoutRoute.json.telemetry.command_success -eq $false -and
  $timeoutRoute.json.telemetry.timeout -eq $true -and
  $timeoutRoute.json.telemetry.latency_ms -le 1500 -and
...
try {
  ...
  $deepSearchRow.ok = (
    ...
    $null -eq $deepSearch.parse_error -and
    $deepSearch.json.status -eq "ok" -and
    $deepSearch.json.telemetry.matches -ge 1 -and
    $deepSearch.json.telemetry.visited_files -gt 20 -and
    $deepSearch.json.telemetry.truncated_by_visit -eq $false -and
...
$cacheDir = Join-Path ([System.IO.Path]::GetTempPath()) ("tokenzero-cache-dir-" + [guid]::NewGuid().ToString("N"))
$cacheFile = Join-Path ([System.IO.Path]::GetTempPath()) ("tokenzero-cache-read-" + [guid]::NewGuid().ToString("N") + ".txt")
try {
  ...
  $cacheDegradeRow.ok = (
    ...
    $null -eq $cacheDegrade.parse_error -and
    $cacheDegrade.json.status -eq "ok" -and
    $cacheDegrade.json.diagnostic.code -eq "cache_write_failed" -and
    $cacheDegrade.json.visible.text.Contains("alpha")
  )
...
if (![string]::IsNullOrWhiteSpace($env:USERPROFILE)) {
  $homeReadFile = Join-Path $env:USERPROFILE ".tokenzero-scorecard-home-read.tmp"
  $homeMarkdownFile = Join-Path $env:USERPROFILE ".tokenzero-scorecard-home-read.md"
  try {
    ...
    $homeReadRow.ok = (
      ...
      $homeRead.json.status -eq "ok" -and
      $homeRead.json.visible.text.Contains("home-ok") -and
      $homeRead.json.accounting.raw_tokens -gt 500 -and
    ...
    $markdownReadRow.ok = (
      ...
      $null -eq $markdownRead.parse_error -and
      $markdownRead.json.status -eq "ok" -and
      $markdownRead.json.content_type -eq "markdown"
    )
    ...
    Remove-Item -Force -ErrorAction SilentlyContinue $homeReadFile
    Remove-Item -Force -ErrorAction SilentlyContinue $homeMarkdownFile
  }
}
...
$noisyRow.ok = (
  $noisy.exit_code -eq 0 -and
  $null -eq $noisy.parse_error -and
  $noisy.json.status -eq "ok" -and
  $noisy.json.telemetry.command_success -eq $true -and
  $noisy.json.telemetry.output_strategy -ne "compact_adaptive_shell" -and
...
$readRow.ok = (
  $read.exit_code -eq 0 -and
  $null -eq $read.parse_error -and
  $read.json.status -eq "ok" -and
  ($read.json.accounting.visible_tokens * 4) -lt $read.json.accounting.raw_tokens
)
if (!$readRow.ok) {
...
$report = [ordered]@{
  ...
  tokenzero_exe = $TokenZeroExe
  thresholds = [ordered]@{
    tiny_latency_ms_max = 100
    tiny_wall_ms_max = 3500
    powershell_variable_wall_ms_max = 3500
    timeout_override_latency_ms_max = 1500
    timeout_override_wall_ms_max = 5000
    tiny_visible_tokens_lte_raw = $true
    noisy_visible_tokens_lt_half_raw = $true
    read_visible_tokens_lt_quarter_raw = $true
  }
  cases = $cases
  failures = $failures
}
