...
try {
  ...
  if (!$SkipVerifier) {
    $completed += Invoke-Checked `
      ...
      -FileName "powershell" `
      -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts/rust_windows_verify.ps1")
  }
  if (!$SkipRehearsal) {
    $completed += Invoke-Checked `
      ...
      -WorkingDirectory $RepoRoot `
      -FileName "powershell" `
      -Arguments @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "scripts/rust_windows_global_rehearsal.ps1")
  }
