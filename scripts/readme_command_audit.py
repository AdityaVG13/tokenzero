...
import argparse; import os; import re; import shlex
import shutil; import subprocess; import sys; import tempfile
from dataclasses import dataclass; from pathlib import Path; AUDIT_SKIP = '<!-- audit:skip -->'; FENCE_RE = re.compile('^\\s*```([A-Za-z0-9_-]+)?\\s*$')
INLINE_RE = re.compile('`([^`\\n]+)`'); KNOWN_BINARIES = {'tokenzero', 'fszero', 'graphzero', 'zerostack', 'node', 'cargo', 'git', 'rustup'}; PROJECT_BINARIES = {'tokenzero', 'fszero', 'graphzero'}; IGNORED_COMMANDS = {'cd', 'export', 'printf', 'echo', 'cat', 'mkdir', 'cp', 'mv', 'rm', 'true', 'false'}
CONTROL_WORDS = {'if', 'then', 'else', 'fi', 'for', 'do', 'done', 'while'}; SAFE_EXEC_ARGS = {'--help', '-h', 'help', '--version', '-V', 'version'}; CARGO_WRAPPER_HELP_SUBCOMMAND = 'check'

@dataclass(frozen=True)
class Candidate:
    text: str; line: int; source: str

@dataclass(frozen=True)
class Finding:
    line: int; message: str; command: str

class Audit:

    def __init__(self, root: Path, readme: Path):
        self.root = root; self.readme = readme; self.help_cache: dict[tuple[str, ...], tuple[int, str]] = {}; self.findings: list[Finding] = []
        self.skips: list[Candidate] = []

    def repo_binary(self, name: str) -> str | None:
        if name in PROJECT_BINARIES:
            release = self.root / 'target' / 'release' / name; debug = self.root / 'target' / 'debug' / name
    ...
    def help_text(self, argv: list[str]) -> tuple[int, str]:
        ...
        env = os.environ.copy(); env.setdefault('NO_COLOR', '1')
        try:
            proc = subprocess.run(argv, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=15, env=env); result = (proc.returncode, proc.stdout)
    ...
    def extract(self) -> list[Candidate]:
        lines = self.readme.read_text(encoding='utf-8').splitlines(); candidates: list[Candidate] = []; active_lang: str | None = None
        for idx, line in enumerate(lines, start=1):
            ...
            for match in INLINE_RE.finditer(line):
                text = match.group(1).strip(); first = first_command_word(text)
    ...
    def verify_args(self, candidate: Candidate, resolved: str, binary_name: str, args: list[str]) -> None:
        ...
        if binary_name == 'cargo':
            ...
            return
        subcommand = first_subcommand(binary_name, args); help_text = top_help
        if subcommand:
    ...
    def verify_cargo(self, candidate: Candidate, resolved: str, args: list[str], top_help: str) -> None:
        delimiter = args.index('--') if '--' in args else len(args); cargo_args = args[:delimiter]
        passthrough = args[delimiter + 1:] if delimiter < len(args) else []
        subcommand = first_subcommand('cargo', cargo_args); help_text = top_help
        if subcommand:
            code, text = self.help_text([resolved, subcommand, '--help'])
            if code != 0 and subcommand not in top_help:
                self.findings.append(Finding(candidate.line, f"unknown subcommand '{subcommand}'", candidate.text))
                return
            help_text = text + '\n' + top_help
        wrapper_text = ''; code, text = self.help_text([resolved, CARGO_WRAPPER_HELP_SUBCOMMAND, '--help'])
        if code == 0:
            wrapper_text = text
...
def normalize_argv(argv: list[str]) -> list[str]:
    out = list(argv)
    while out and re.match('^[A-Za-z_][A-Za-z0-9_]*=.*$', out[0]):
        out.pop(0)
...
def commands_from_shell_line(line: str) -> list[str]:
    ...
    if not raw or raw.startswith('#'):
        return []
    raw = raw.replace('\\\n', ' '); pieces = re.split('\\s*(?:&&|\\|\\||\\|)\\s*', raw); commands: list[str] = []
    for piece in pieces:
        piece = piece.strip()
...
def self_check() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp); readme = root / 'README.md'
        readme.write_text('# Scratch\n\n```bash\nzerostack serve\n```\n\n<!-- audit:skip -->\n`zerostack serve`\n', encoding='utf-8')
        audit = Audit(root, readme); code = audit.run()
        if code == 0:
        ...
        if not audit.skips:
            ...
            return 1
        print('OK: self-check rejected fake zerostack serve and reported skip')
        return 0

def main() -> int:
    parser = argparse.ArgumentParser(description='Audit README command claims'); parser.add_argument('--readme', type=Path, default=None); parser.add_argument('--self-check', action='store_true'); args = parser.parse_args()
    if args.self_check:
        return self_check()
    root = Path(__file__).resolve().parents[1]; readme = args.readme or root / 'README.md'
    return Audit(root, readme).run()
