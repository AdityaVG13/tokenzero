...
TOTAL_FILES=$((NUM_DIRS*FILES_PER_DIR)); TOTAL_LINES=$((TOTAL_FILES*LINES_PER_FILE)); WORK_DIR="$(mktemp -d /tmp/tz-million.XXXXXX)"; SYNTH="$WORK_DIR/repo"; TMP_JSON="$WORK_DIR/out.json"; TMP_RAW="$WORK_DIR/raw.out"
trap 'rm -rf "$WORK_DIR"' EXIT
log() { printf '[million-nav] %s\n' "$*" >&2; }; now_ms() { "${H[@]}" now_ms; }
tz_run() {
  local cmd="$1" start end; start=$(now_ms); eval "$cmd" >"$TMP_JSON" 2>/dev/null || true; end=$(now_ms); "${H[@]}" tz_metrics "$TMP_JSON" "$((end-start))"
}
raw_run() {
  local cmd="$1" start end; start=$(now_ms); eval "$cmd" >"$TMP_RAW" 2>/dev/null || true; end=$(now_ms); printf '%s\t%s' "$(wc -c <"$TMP_RAW" | tr -d ' ')" "$((end-start))"
}
emit_header() { printf '| # | Task | Tool | visible_tokens | raw_tokens | wall_ms | output_bytes | notes |\n|---|------|------|---:|---:|---:|---:|------|\n'; }
emit_tz()  { printf '| %s | `%s` | `tokenzero` | %s | %s | %s | — | %s |\n' "$1" "$2" "$3" "$4" "$5" "$6"; }; emit_raw() { printf '| %s | `%s` | `raw-cli` | — | — | %s | %s | %s |\n' "$1" "$2" "$4" "$3" "$5"; }

log "binary: $BIN"; log "budget: $BUDGET visible_tokens"; log "generating $TOTAL_LINES lines across $TOTAL_FILES files in $SYNTH ..."
...
EDIT_FILE="$WORK_DIR/edit_target.rs"; cp "$NEEDLE_FILE" "$EDIT_FILE"; TOTAL_VISIBLE=0; emit_header
...
vis_b=$((vis_b1+vis_b2)); raw_b=$((raw_b1+raw_b2)); ms_b=$((ms_b1+ms_b2)); TOTAL_VISIBLE=$((TOTAL_VISIBLE+vis_b)); emit_tz B grep_expand "$vis_b" "$raw_b" "$ms_b" "find+expand"
...
vis_c=$((vis_c1+vis_c2+vis_c3)); raw_c=$((raw_c1+raw_c2+raw_c3)); ms_c=$((ms_c1+ms_c2+ms_c3)); TOTAL_VISIBLE=$((TOTAL_VISIBLE+vis_c)); emit_tz C tree_glob_read "$vis_c" "$raw_c" "$ms_c" "tree+glob+read"
...
vis_d=$((vis_d1+vis_d2+vis_d3+vis_d4)); raw_d=$((raw_d1+raw_d2+raw_d3+raw_d4)); ms_d=$((ms_d1+ms_d2+ms_d3+ms_d4)); TOTAL_VISIBLE=$((TOTAL_VISIBLE+vis_d)); emit_tz D grep_expand_edit_verify "$vis_d" "$raw_d" "$ms_d" "4-step"
