...
EDIT_TMP=$(mktemp $ROOT/.tz-bakeoff-edit.XXXXXX); CAP_FILE=$(mktemp /tmp/tz-bakeoff-cap.XXXXXX); SHELL_TMP=$(mktemp /tmp/tz-bakeoff-shell.XXXXXX)
trap 'rm -f $EDIT_TMP $CAP_FILE $SHELL_TMP' EXIT
...
cli_one() {
  ...
  local q; if [[ $status -ne 0 ]]; then q=FAIL; elif [[ $task == none ]]; then q=PASS; else q=$(quality_check "$task" "$out"); fi; printf '%s\t%s\t%s\t%s\t%s\n' "$(tok "$cmd_str")" "$out_tok" 1 "$((end-start))" "$q"
}
sum_rows() {
...
run_codemode() {
  local task="$1" plan="$2" start end out status=0; start=$(now_ms); out=$("$BIN" codemode --json "$plan") || status=$?; end=$(now_ms)
...
emit_cli "$task" cli_one "$task" "$BIN read $CARGO_FILE --end-line 20 --json" "$BIN" read "$CARGO_FILE" --end-line 20 --json; run_codemode "$task" 'const f = await zero.read("'"$CARGO_FILE"'", { end_line: 20 }); return f;'

task=search_filter; emit_row "$task" MCP-schema "$(mcp_tok find,grep)" 0 1 "$MCP_WALL" simulated
emit_cli "$task" cli_one "$task" "$BIN find TokenZero $CORPUS_FILE --json" "$BIN" find TokenZero "$CORPUS_FILE" --json; run_codemode "$task" 'const h = await zero.find("TokenZero", "'"$CORPUS_FILE"'"); return h;'
...
setup_edit; run_codemode "$task" 'const e = await zero.edit("'"$EDIT_TMP"'", [{ find: "beta", replace: "BETA" }]); const f = await zero.read("'"$EDIT_TMP"'"); return f;'

task=multi_step_nav; emit_row "$task" MCP-schema "$(mcp_tok tree,read,find)" 0 1 "$MCP_WALL" simulated
row=$(
  cli_one none "$BIN tree $ROOT --depth 2 --json" "$BIN" tree "$ROOT" --depth 2 --json; cli_one none "$BIN read $CARGO_FILE --json" "$BIN" read "$CARGO_FILE" --json
...
$BIN run --json -- find . -maxdepth 1 -name Cargo.toml > $SHELL_TMP; SHELL_REF=$("${H[@]}" first_blob_ref "$SHELL_TMP"); [[ -n "$SHELL_REF" ]]
row=$(
  cli_one none "$BIN run --json -- find . -maxdepth 1 -name Cargo.toml" \
    "$BIN" run --json -- find . -maxdepth 1 -name Cargo.toml
  cli_one "$task" "$BIN expand $SHELL_REF" "$BIN" expand "$SHELL_REF"
)
IFS=$'\t' read -r i o t w q <<<"$(printf '%s' "$row" | sum_rows)"; emit_row "$task" CLI "$i" "$o" "$t" "$w" "$q"
