...
def measure(label: str, root: Path, cache_dir: Path) -> dict[str, object]:
    ...
    command = [str(BIN), "session-open", "--root", str(root), "--cache-path", str(cache), "--json"]
    initialized = H.run_json(command)
    recorded = H.run_json(command)
    boot = recorded["raw_json"]
    components = boot["telemetry"]
    component_sum = sum(int(components[name]) for name in ("manifest", "delta", "toc_working_set", "other"))
    ...
    # drop helper-only keys to preserve boot-cost evidence shape
...
def run(label: str) -> Path:
    ...
    with H.heavy_guard(f"python3 benchmarks/boot-cost.py --label {label}"):
        with tempfile.TemporaryDirectory(prefix="tokenzero-boot-cost-") as raw:
            ...
            synthetic.mkdir()
            H.synthetic_tree(synthetic, SYNTHETIC_FILES)
            result = {
