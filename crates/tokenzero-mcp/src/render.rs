...
/// Stable seen-set ack for ref-based expand delivery.
pub(crate) fn identical_expand_ack(ref_id: &str) -> String {
    format!("identical to {ref_id} (unchanged)")
}
...
pub(crate) fn expand_since_diff_text(since_ref: &str, target_ref: &str, diff_body: &str) -> String {
    format!("# expand {target_ref} — diff since {since_ref}
{diff_body}")
}
