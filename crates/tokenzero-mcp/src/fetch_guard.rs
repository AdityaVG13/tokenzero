...
mod tests {
    ...
    #[test]
    fn blocked_literal_matrix() {
        for url in [
            "http://127.0.0.1/", "http://10.1.2.3/x", "http://172.16.9.1:8080/",
            "http://192.168.0.10/", "http://169.254.169.254/latest/meta-data/",
            "http://100.64.1.1/", "http://0.0.0.0/", "http://[::1]/",
            "http://[fe80::1]/", "http://[fd00::2]/", "http://224.0.0.1/",
            "http://240.0.0.1/", "http://198.18.0.1/", "http://[::ffff:127.0.0.1]/",
        ] {
            assert_eq!(validate(url).unwrap_err().code, "fetch_blocked", "{url}");
        }
    }

    #[test]
    fn resolution_allow_and_deny_precedence() {
        let blocked = validate("http://localhost:9999/admin").unwrap_err();
        assert_eq!(blocked.code, "fetch_blocked");
        assert!(blocked.message.contains("loopback"), "{}", blocked.message);

        let allow = ["localhost".to_string()];
        let target = validate_fetch_target("http://localhost:3000/", &allow, &[]).unwrap();
        assert_eq!((&*target.host, target.port, target.pinned_ip), ("localhost", 3000, None));
        let deny = ["localhost".to_string()];
        assert!(validate_fetch_target("http://localhost:3000/", &allow, &deny)
            .unwrap_err().message.contains("denied"));
    }
    ...
    #[test]
    fn malformed_url_matrix() {
        for url in ["http://trusted.com@127.0.0.1/", "ftp://example.com/", "http:///path", "http://[::1/"] {
            assert!(validate(url).is_err(), "{url} must be rejected");
        }
    }

    #[test]
    fn fetch_meta_matrix() {
        for (stdout, body, code, redirect) in [
            (format!("hello body\n{FETCH_META_MARKER} 302 https://next.example.com/x\n"), "hello body", Some(302), Some("https://next.example.com/x")),
            ("no marker here\n".to_string(), "no marker here\n", None, None),
            (format!("body\n{FETCH_META_MARKER} 200 "), "body", Some(200), None),
        ] {
            let actual = split_fetch_meta(&stdout);
            assert_eq!(actual.0, body);
            assert_eq!(actual.1, code);
            assert_eq!(actual.2.as_deref(), redirect);
        }
    }
}
