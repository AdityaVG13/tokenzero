...
pub enum OperationClass { ReadOnly, ReversibleStoreMutation, IrreversibleExternal, Unknown }
...
pub fn classify_method(method: &str) -> OperationClass {
    let bare = method.strip_prefix("zero.token.")
        .or_else(|| method.strip_prefix("zero."))
        .or_else(|| method.strip_prefix("tz_"))
        .unwrap_or(method).replace('-', "_");
    match bare.as_str() {
        "read" | "find" | "grep" | "glob" | "tree" | "expand" | "expandMany"
        | "expand_many" | "dedupe" | "mem" | "recall" | "rewrite" | "discover"
        | "pick" | "filter_lines" | "count" | "first" | "verdict" | "raw"
        | "count_tokens" | "assert" | "codemode.search" | "codemode.describe"
        | "codemode.limits" | "codemode.journalDoctor" | "journalDoctor" | "journal_doctor"
        | "codemode.journalInspect" | "journalInspect" | "journal_inspect"
        | "codemode.journalResume" | "journalResume" | "journal_resume"
        | "search" | "describe" | "limits" => OperationClass::ReadOnly,
        "edit" | "codemode.journalRollback" | "journalRollback" | "journal_rollback" | "compact" | "compactMany" | "compact_many" | "compact_max"
        | "ingest" | "cache_pack" | "store_put" | "store_alias" | "migration_apply" => OperationClass::ReversibleStoreMutation,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum JournalState { Prepared, Applying, Committed, RollingBack, RolledBack, ManualIntervention }
impl JournalState { pub fn is_resolved(self) -> bool { matches!(self, Self::Committed | Self::RolledBack) } }

#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum StepState { Prepared, Applying, Applied, RollingBack, RolledBack, Skipped }
...
pub struct JournalOperation {
    ...
    #[serde(skip_serializing_if = "Option::is_none")] pub precondition_digest: Option<String>,
    #[serde(skip_serializing_if = "Option::is_none")] pub precondition_exists: Option<bool>,
    #[serde(skip_serializing_if = "Option::is_none")] pub postcondition_digest: Option<String>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")] pub undo_refs: Vec<String>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")] pub compensation_refs: Vec<String>,
    #[serde(skip_serializing_if = "Option::is_none")] pub size: Option<u64>,
    pub state: StepState,
    #[serde(skip_serializing_if = "Option::is_none")] pub diagnostic: Option<String>,
}
...
pub struct PlanJournal {
    ...
    #[serde(skip_serializing_if = "Option::is_none")] pub original_error: Option<String>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")] pub rollback_errors: Vec<String>,
...
pub struct CombinedRollbackError { pub original: String, pub rollback: Vec<String> }
impl std::fmt::Display for CombinedRollbackError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "original error: {}", self.original)?;
        if !self.rollback.is_empty() { write!(f, "; rollback errors: {}", self.rollback.join(" | "))?; }
        Ok(())
    }
}
...
pub fn journal_root(cache_path: &Path) -> PathBuf {
    cache_path.parent().unwrap_or_else(|| Path::new(".")).join("plan-journals")
}

pub fn begin_plan(
    cache_path: &Path, project_root: &Path, plan_id: &str, execution_id: &str,
    specs: Vec<OperationSpec>, atomic_requested: bool,
) -> Result<BeginOutcome, String> {
    begin_plan_inner(cache_path, project_root, plan_id, execution_id, specs, atomic_requested, None)
}

#[cfg(test)]
fn begin_plan_with_fault(
    cache_path: &Path, project_root: &Path, plan_id: &str, execution_id: &str,
    specs: Vec<OperationSpec>, atomic_requested: bool, fault_at: &str,
) -> Result<BeginOutcome, String> {
    begin_plan_inner(
        cache_path, project_root, plan_id, execution_id, specs, atomic_requested,
...
fn begin_plan_inner(
    cache_path: &Path, project_root: &Path, plan_id: &str, execution_id: &str,
    specs: Vec<OperationSpec>, atomic_requested: bool, fault_at: Option<String>,
) -> Result<BeginOutcome, String> {
    let classes: Vec<_> = specs.iter().map(|spec| classify_method(&spec.method)).collect();
    let unsafe_methods: Vec<_> = specs.iter().zip(&classes)
        .filter(|(_, class)| matches!(class, OperationClass::IrreversibleExternal | OperationClass::Unknown))
        .map(|(spec, _)| spec.method.clone()).collect();
    if !unsafe_methods.is_empty() {
        let reason = format!("plan contains non-rollbackable or unknown operations: {}", unsafe_methods.join(", "));
        if atomic_requested { return Err(format!("atomic plan rejected: {reason}")); }
        record_downgrade(
            cache_path, project_root, plan_id, execution_id, &specs, &classes, &reason,
        )?;
        return Ok(BeginOutcome::Downgraded { reason });
    }
    if !classes.contains(&OperationClass::ReversibleStoreMutation) { return Ok(BeginOutcome::Disabled); }
    ...
    FileExt::try_lock(&lock).map_err(|err| format!("plan conflict: another mutation plan holds {}: {err}", lock_path.display()))?;
    let path = root.join(format!("{}.json", safe_execution_id(execution_id)));
    if path.exists() {
        let journal = read_journal(&path)?;
        if journal.execution_id != execution_id || journal.plan_id != plan_id { return Err("journal identity collision".into()); }
        if journal.state == JournalState::Committed {
            remove_pins(&root, &journal)?;
            return Ok(BeginOutcome::AlreadyCommitted);
        }
        if journal.state == JournalState::RolledBack { return Err("execution was already rolled back; use a new execution_id".into()); }
        return Ok(BeginOutcome::Transaction(JournalTransaction { root, path, lock, journal, fault_at: fault_at.clone() }));
    }

    let created = now_ms();
    let operations = specs.into_iter().enumerate().map(|(index, spec)| {
        let pre = spec.precondition_digest.clone().unwrap_or_else(|| "none".into());
        JournalOperation {
            index, id: spec.id, method: spec.method.clone(), classification: classes[index],
            idempotency_key: sha256_bytes(format!("{execution_id}:{index}:{}:{pre}", spec.method).as_bytes()),
            target: spec.target.map(|path| path.to_string_lossy().into_owned()),
            precondition_digest: spec.precondition_digest, precondition_exists: spec.precondition_exists,
            postcondition_digest: spec.postcondition_digest, undo_refs: spec.undo_refs, compensation_refs: Vec::new(),
            size: spec.size, state: StepState::Prepared, diagnostic: None,
        }
    }).collect();
    let journal = PlanJournal {
        version: PLAN_JOURNAL_VERSION.into(), plan_id: plan_id.into(), execution_id: execution_id.into(),
        project_id: sha256_bytes(project_root.to_string_lossy().as_bytes()),
        store_id: sha256_bytes(cache_path.to_string_lossy().as_bytes()), atomic: true,
        downgrade_reason: None, state: JournalState::Prepared, operations,
        original_error: None, rollback_errors: Vec::new(), created_at_ms: created, updated_at_ms: created,
    };
    write_pins(&root, &journal)?;
    persist_journal(&path, &journal)?;
    let tx = JournalTransaction { root, path, lock, journal, fault_at };
    tx.check_fault("prepare")?;
    Ok(BeginOutcome::Transaction(tx))
}
...
fn record_downgrade(
    ...
    FileExt::try_lock(&lock).map_err(|err| {
        format!("plan conflict: another mutation plan holds {}: {err}", lock_path.display())
    })?;
    let path = root.join(format!("{}.json", safe_execution_id(execution_id)));
    if path.exists() {
    ...
    let created = now_ms();
    let operations = specs.iter().enumerate().map(|(index, spec)| JournalOperation {
        index,
        id: spec.id.clone(),
        ...
        target: spec.target.as_ref().map(|path| path.to_string_lossy().into_owned()),
...
impl JournalTransaction {
    pub fn journal(&self) -> &PlanJournal { &self.journal }
    #[cfg(test)] pub fn set_fault(&mut self, boundary: impl Into<String>) { self.fault_at = Some(boundary.into()); }
    fn check_fault(&self, boundary: &str) -> Result<(), String> {
        if self.fault_at.as_deref() == Some(boundary) { Err(format!("fault injected at {boundary}")) } else { Ok(()) }
    }
    fn persist(&mut self) -> Result<(), String> {
        self.journal.updated_at_ms = now_ms(); persist_journal(&self.path, &self.journal)
    }
    pub fn step_needs_apply(&self, index: usize) -> bool {
        self.journal.operations.get(index).is_some_and(|op| !matches!(op.state, StepState::Applied | StepState::RolledBack | StepState::Skipped))
    }
    pub fn verify_precondition(&self, index: usize) -> Result<(), String> {
        let op = self.journal.operations.get(index).ok_or_else(|| "journal step missing".to_string())?;
        let Some(target) = op.target.as_deref() else { return Ok(()); };
        let actual = current_digest(Path::new(target)).map_err(|err| format!("read precondition target: {err}"))?;
        match (&op.precondition_digest, op.precondition_exists, actual) {
            ...
            (_, _, actual) => Err(format!("CAS precondition failed for {target}: expected {:?}, actual {:?}", op.precondition_digest, actual)),
        }
    }
    pub fn mark_applying(&mut self, index: usize) -> Result<(), String> {
        self.verify_precondition(index)?; self.journal.state = JournalState::Applying;
        self.journal.operations[index].state = StepState::Applying; self.persist()
    }
    pub fn mark_applied(&mut self, index: usize, postcondition_digest: Option<String>, compensation_refs: Vec<String>) -> Result<(), String> {
        let op = self.journal.operations.get_mut(index).ok_or_else(|| "journal step missing".to_string())?;
        if let (Some(expected), Some(actual)) = (&op.postcondition_digest, &postcondition_digest) {
            if expected != actual {
                return Err(format!("postcondition CAS failed for step {index}: expected {expected}, actual {actual}"));
            }
        }
        if op.postcondition_digest.is_none() { op.postcondition_digest = postcondition_digest; }
        op.compensation_refs = compensation_refs; op.state = StepState::Applied;
        self.persist()?; self.check_fault(&format!("apply-step-{index}"))
    }
    pub fn commit(mut self) -> Result<PlanJournal, String> {
        self.journal.state = JournalState::Committed; self.persist()?; self.check_fault("commit")?;
        remove_pins(&self.root, &self.journal)?; enforce_retention(&self.root)?;
        let _ = FileExt::unlock(&self.lock); Ok(self.journal.clone())
    }
    pub fn rollback<F>(&mut self, original_error: impl Into<String>, mut undo: F) -> Result<(), CombinedRollbackError>
    where F: FnMut(&JournalOperation) -> Result<(), String> {
        let original = bounded(&original_error.into()); self.journal.original_error = Some(original.clone());
        self.journal.state = JournalState::RollingBack;
        if let Err(err) = self.persist() { return Err(CombinedRollbackError { original, rollback: vec![err] }); }
        let mut errors = Vec::new();
        for index in (0..self.journal.operations.len()).rev() {
            if !matches!(self.journal.operations[index].state, StepState::Applied | StepState::Applying | StepState::RollingBack) { continue; }
            self.journal.operations[index].state = StepState::RollingBack;
            if let Err(err) = self.persist() { errors.push(err); break; }
            ...
            match undo(&snapshot) {
                Ok(()) => {
                    ...
                    if let Err(err) = self.check_fault(&format!("rollback-step-{index}")) {
                        ...
                        return Err(CombinedRollbackError { original, rollback: vec![err] });
                    }
                }
                Err(err) => {
                    let message = bounded(&format!("step {index}: {err}"));
                    self.journal.operations[index].diagnostic = Some(message.clone()); errors.push(message); let _ = self.persist();
                }
            }
        }
        ...
        self.journal.state = if errors.is_empty() { JournalState::RolledBack } else { JournalState::ManualIntervention };
        if let Err(err) = self.persist() { errors.push(err); }
        if errors.is_empty() { if let Err(err) = remove_pins(&self.root, &self.journal) { errors.push(err); } let _ = enforce_retention(&self.root); }
        if errors.is_empty() { Ok(()) } else { Err(CombinedRollbackError { original, rollback: errors }) }
    }
}
impl Drop for JournalTransaction { fn drop(&mut self) { let _ = FileExt::unlock(&self.lock); } }

pub fn doctor(cache_path: &Path) -> JournalDoctorReport {
    let root = journal_root(cache_path);
    let mut report = JournalDoctorReport { schema_version: PLAN_JOURNAL_VERSION.into(), unresolved: Vec::new(), resolved_count: 0, corrupt: Vec::new() };
    let Ok(entries) = fs::read_dir(&root) else { return report; };
            ...
            Ok(journal) if journal.state.is_resolved() => {
                ...
                if let Err(err) = remove_pins(&root, &journal) {
                    report.corrupt.push(format!(
                        ...
                        path.display(), bounded(&err),
                    ));
                }
            },
            Ok(journal) => report.unresolved.push(JournalDoctorEntry {
                execution_id: journal.execution_id, state: journal.state,
                journal_path: path.to_string_lossy().into_owned(), next_step: next_step(journal.state).into(),
            }),
            Err(err) => report.corrupt.push(format!("{}: {}", path.display(), bounded(&err))),
        }
    }
    report.unresolved.sort_by(|a, b| a.execution_id.cmp(&b.execution_id)); report.corrupt.sort(); report
}
pub fn inspect(cache_path: &Path, execution_id: &str) -> Result<PlanJournal, String> {
    read_journal(&journal_root(cache_path).join(format!("{}.json", safe_execution_id(execution_id))))
}
pub fn open_unresolved(cache_path: &Path, execution_id: &str) -> Result<JournalTransaction, String> {
    let root = journal_root(cache_path);
    fs::create_dir_all(&root).map_err(|err| format!("create journal directory: {err}"))?;
    let lock_path = root.join("mutation.lock");
    let lock = OpenOptions::new().create(true).read(true).write(true).open(&lock_path)
    ...
    if journal.state.is_resolved() {
        return Err(format!("journal {execution_id} is already resolved as {:?}", journal.state));
    }
    Ok(JournalTransaction { root, path, lock, journal, fault_at: None })
}
pub fn doctor_json(cache_path: &Path) -> Value {
    serde_json::to_value(doctor(cache_path)).unwrap_or_else(|_| json!({"schema_version": PLAN_JOURNAL_VERSION, "unresolved": []}))
}
fn next_step(state: JournalState) -> &'static str {
    match state {
        ...
        JournalState::ManualIntervention => "inspect original_error and rollback_errors; repair, then rollback",
        JournalState::Committed | JournalState::RolledBack => "resolved",
    }
}
fn bounded(value: &str) -> String {
    if value.len() <= MAX_DIAGNOSTIC_BYTES { return value.into(); }
    let mut end = MAX_DIAGNOSTIC_BYTES; while !value.is_char_boundary(end) { end -= 1; }
    format!("{}...[truncated]", &value[..end])
}
fn safe_execution_id(value: &str) -> String {
    let tail = value.strip_prefix("cm://exec/").unwrap_or(value);
    let safe: String = tail.chars().map(|ch| if ch.is_ascii_alphanumeric() || matches!(ch, '-' | '_' | '.') { ch } else { '_' }).take(128).collect();
    if safe.is_empty() { sha256_bytes(value.as_bytes()) } else { safe }
}
fn read_journal(path: &Path) -> Result<PlanJournal, String> {
    let bytes = fs::read(path).map_err(|err| format!("read journal: {err}"))?;
    let journal: PlanJournal = serde_json::from_slice(&bytes).map_err(|err| format!("parse journal: {err}"))?;
    if journal.version != PLAN_JOURNAL_VERSION { return Err(format!("unsupported journal version {}", journal.version)); }
    Ok(journal)
}
fn persist_journal(path: &Path, journal: &PlanJournal) -> Result<(), String> {
    let bytes = serde_json::to_vec_pretty(journal).map_err(|err| format!("serialize journal: {err}"))?;
...
pub(crate) fn atomic_write(path: &Path, bytes: &[u8]) -> io::Result<()> {
    ...
    let tmp = parent.join(format!(".{}.{}.tmp", path.file_name().and_then(|v| v.to_str()).unwrap_or("journal"), std::process::id()));
    let result = (|| { let mut file = OpenOptions::new().create_new(true).write(true).open(&tmp)?; file.write_all(bytes)?; file.sync_all()?; fs::rename(&tmp, path)?; File::open(parent)?.sync_all()?; Ok(()) })();
...
fn pin_hash(reference: &str) -> Option<String> {
    ...
    let hash = if value.len() == 65 && value.starts_with('b') { &value[1..] } else { value };
    (hash.len() == 64 && hash.bytes().all(|b| b.is_ascii_hexdigit() && !b.is_ascii_uppercase())).then(|| hash.into())
}
fn pin_path(root: &Path, execution_id: &str, index: usize, hash: &str) -> PathBuf {
    root.join("pins").join(format!("plan-{}-{index}-{}.json", safe_execution_id(execution_id), &hash[..12]))
}
fn write_pins(root: &Path, journal: &PlanJournal) -> Result<(), String> {
    for op in &journal.operations { for reference in &op.undo_refs {
        let Some(hash) = pin_hash(reference) else { continue; };
        ...
        atomic_write(&pin_path(root, &journal.execution_id, op.index, &hash), &bytes).map_err(|err| format!("persist undo pin: {err}"))?;
    }} Ok(())
}
fn remove_pins(root: &Path, journal: &PlanJournal) -> Result<(), String> {
    for op in &journal.operations { for reference in &op.undo_refs {
        let Some(hash) = pin_hash(reference) else { continue; }; let path = pin_path(root, &journal.execution_id, op.index, &hash);
        match fs::remove_file(&path) { Ok(()) => {}, Err(err) if err.kind() == io::ErrorKind::NotFound => {}, Err(err) => return Err(format!("remove resolved pin {}: {err}", path.display())) }
    }} Ok(())
}
fn enforce_retention(root: &Path) -> Result<(), String> {
    let mut resolved = Vec::new();
    let entries = match fs::read_dir(root) { Ok(v) => v, Err(err) if err.kind() == io::ErrorKind::NotFound => return Ok(()), Err(err) => return Err(err.to_string()) };
    for entry in entries.flatten() { let path = entry.path(); if path.extension().and_then(|v| v.to_str()) != Some("json") { continue; }
        if let Ok(journal) = read_journal(&path) { if journal.state.is_resolved() { resolved.push((journal.updated_at_ms, entry.metadata().map(|m| m.len()).unwrap_or(0), path)); } }
    }
    resolved.sort_by_key(|item| item.0); let mut total: u64 = resolved.iter().map(|item| item.1).sum(); let mut count = resolved.len();
    for (_, bytes, path) in resolved { if count <= MAX_RESOLVED_JOURNALS && total <= MAX_RESOLVED_BYTES { break; } fs::remove_file(&path).map_err(|err| format!("prune resolved journal {}: {err}", path.display()))?; count -= 1; total = total.saturating_sub(bytes); }
    Ok(())
}
...
fn rfc3339_now() -> String {
    ...
    let (year, month, day) = civil_from_days((seconds / 86_400) as i64); let rem = seconds % 86_400;
...
fn civil_from_days(days: i64) -> (i64, u32, u32) {
    ...
    let yoe = (doe - doe / 1460 + doe / 36_524 - doe / 146_096) / 365; let mut year = yoe + era * 400;
    let doy = doe - (365 * yoe + yoe / 4 - yoe / 100); let mp = (5 * doy + 2) / 153; let day = doy - (153 * mp + 2) / 5 + 1;
    let month = mp + if mp < 10 { 3 } else { -9 }; year += if month <= 2 { 1 } else { 0 }; (year, month as u32, day as u32)
}
...
mod tests {
    ...
    fn journal_state_machine_fault_boundaries_restart_classification() {
        ...
        let error = begin_plan_with_fault(
            &cache, temp.path(), "plan-a", execution, vec![spec(0)], true, "prepare",
        ).unwrap_err();
        ...
        let mut tx = match begin_plan(&cache, temp.path(), "plan-b", rollback_execution, vec![spec(0)], true).unwrap() {
    ...
    fn journal_idempotent_replay_has_single_effect() {
        ...
        let mut tx = match begin_plan(&cache, temp.path(), "plan", execution, vec![spec(0)], true).unwrap() {
            BeginOutcome::Transaction(tx) => tx,
            other => panic!("unexpected outcome: {other:?}"),
        };
        tx.mark_applying(0).unwrap();
        tx.mark_applied(0, Some(sha256_bytes(b"once")), Vec::new()).unwrap();
        tx.commit().unwrap();
        assert!(matches!(
    ...
    fn journal_rollback_is_reverse_order_and_preserves_both_errors() {
        ...
        let mut tx = match begin_plan(&cache, temp.path(), "plan", execution, vec![spec(0), spec(1)], true).unwrap() {
            BeginOutcome::Transaction(tx) => tx,
            other => panic!("unexpected outcome: {other:?}"),
        };
        for index in 0..2 {
        ...
        let error = tx.rollback("apply failed", |operation| {
            order.push(operation.index);
            if operation.index == 0 { Err("undo failed".to_string()) } else { Ok(()) }
        }).unwrap_err();
    ...
    fn journal_mixed_shell_plan_rejects_atomic_or_records_downgrade() {
        let temp = tempdir().unwrap();
        let cache = cache(temp.path());
        let specs = vec![spec(0), OperationSpec {
            id: "shell".to_string(), method: "zero.shell".to_string(), target: None,
            precondition_digest: None, precondition_exists: None, postcondition_digest: None, undo_refs: Vec::new(), size: None,
        }];
    ...
    fn journal_conflict_child() {
        let Some(root) = std::env::var_os("TOKENZERO_JOURNAL_CONFLICT_ROOT") else { return; };
        let root = PathBuf::from(root);
        let error = begin_plan(&cache(&root), &root, "plan", "cm://exec/child", vec![spec(0)], true).unwrap_err();
    ...
    fn journal_multi_process_conflict_fails_before_overwrite() {
        let temp = tempdir().unwrap();
        let _held = match begin_plan(&cache(temp.path()), temp.path(), "plan", "cm://exec/parent", vec![spec(0)], true).unwrap() {
            BeginOutcome::Transaction(tx) => tx,
            other => panic!("unexpected outcome: {other:?}"),
        };
    ...
    fn journal_doctor_lists_unresolved_and_pins_redacted_undo_refs() {
        ...
        let tx = match begin_plan(&cache, temp.path(), "plan", execution, vec![spec(0)], true).unwrap() {
            BeginOutcome::Transaction(tx) => tx,
            other => panic!("unexpected outcome: {other:?}"),
        };
        ...
        let pin_paths = fs::read_dir(&pin_dir).unwrap()
    ...
    fn journal_json_plan_commit_replay_and_restart_smoke() {
        ...
        assert_eq!(second.status, CodeModeStatus::Completed, "{:?}", second.error);
        assert_eq!(fs::read_to_string(&path).unwrap(), "after-secret-body");
    ...
    fn journal_fault_injection_covers_each_apply_and_rollback_step() {
        ...
        for fault_index in 0..3 {
            ...
            let mut tx = match begin_plan(
                &cache, temp.path(), "apply-plan", &execution,
                vec![spec(0), spec(1), spec(2)], true,
            ).unwrap() {
                BeginOutcome::Transaction(tx) => tx,
                other => panic!("unexpected outcome: {other:?}"),
            };
            for index in 0..=fault_index {
                ...
                let result = tx.mark_applied(index, Some(sha256_bytes(format!("post-{index}").as_bytes())), Vec::new());
                if index == fault_index { assert!(result.is_err()); } else { result.unwrap(); }
            }
            drop(tx);
            let journal = inspect(&cache, &execution).unwrap();
        ...
        for fault_index in 0..3 {
            ...
            let mut tx = match begin_plan(
                &cache, temp.path(), "rollback-plan", &execution,
                vec![spec(0), spec(1), spec(2)], true,
            ).unwrap() {
                BeginOutcome::Transaction(tx) => tx,
                other => panic!("unexpected outcome: {other:?}"),
    ...
    fn journal_cas_conflict_never_clobbers_newer_data() {
        ...
        let mut tx = match begin_plan(
            &cache, temp.path(), "cas-before", "cm://exec/cas-before", vec![cas_spec.clone()], true,
        ).unwrap() {
            BeginOutcome::Transaction(tx) => tx,
            other => panic!("unexpected outcome: {other:?}"),
        };
        ...
        fs::write(&target, "old").unwrap();
        let mut tx = match begin_plan(
            &cache, temp.path(), "cas-rollback", "cm://exec/cas-rollback", vec![cas_spec], true,
        ).unwrap() {
            BeginOutcome::Transaction(tx) => tx,
            other => panic!("unexpected outcome: {other:?}"),
        };
        ...
        let error = tx.rollback("later failure", |operation| {
    ...
    fn journal_classification_table_covers_policy_boundaries() {
        ...
        assert_eq!(classify_method("zero.edit"), OperationClass::ReversibleStoreMutation);
        assert_eq!(classify_method("zero.shell"), OperationClass::IrreversibleExternal);
        assert_eq!(classify_method("not.in.descriptor"), OperationClass::Unknown);
        assert_eq!(classify_descriptor_tool("tz_ingest"), OperationClass::ReversibleStoreMutation);
