...
tool_table! {
    ...
    Expand => { name: "tz_expand", cluster: "material", capabilities: &["expand", "exact-refs", "fragment-selectors", "symbol-anchors", "diff-baseline", "shared-cas"], summary: "Recover exact bytes from a tz://, fz://, or gz:// ref, optionally narrowed by line range, selector, or symbol.", doc: tool_description( "Recover exact content from `tz://`, `fz://`, or `gz://` blob refs (same-store scheme alias; shared-CAS expand when a compatible ZeroStack CAS / sibling store is attached; multi-OS/proven cross-engine portability not advertised pending multi-OS ZeroRef evidence; non-blob portable refs unsupported) with optional ranges or anchors.", "ref: copy a ref returned by read/find/tree/shell/ingest or sibling ZeroStack engines. selector/start_line/end_line: use only when narrowing recovery.", "Use whenever compact output omitted needed detail. NOT for arbitrary file paths; use `read`.", "Do expand refs instead of re-running expensive commands. Do use line ranges for large file refs. Don't invent refs.", "Common mistakes: passing `path` instead of `ref`; using stale refs from another workspace; expanding whole huge refs unnecessarily.", "Idempotent. Safe to retry.", ), input_schema: expand_schema(), arg_aliases: catalog_json!({}), },
