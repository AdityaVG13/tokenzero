...
impl<'a> ZipTestEntry<'a> {
    fn base(name: &'a str, data: &'a [u8], external_attrs: u32) -> Self {
        Self {
            name,
            data,
            method: 0,
    ...
    pub(crate) fn file(name: &'a str, data: &'a [u8]) -> Self {
        Self::base(name, data, 0o100644 << 16)
    }
    pub(crate) fn symlink(name: &'a str, target: &'a [u8]) -> Self {
        Self::base(name, target, 0o120777 << 16)
    }
    pub(crate) fn with_method(mut self, method: u16) -> Self { self.method = method; self }
    pub(crate) fn with_flags(mut self, flags: u16) -> Self { self.flags |= flags; self }
    pub(crate) fn with_local_name(mut self, local_name: &'a str) -> Self { self.local_name = Some(local_name); self }
    ...
    pub(crate) fn with_zip64_extra_fields(mut self) -> Self { self.force_zip64 = true; self }
    ...
    pub(crate) fn with_central_extra(mut self, extra: Vec<u8>) -> Self { self.central_extra = extra; self }
    pub(crate) fn with_local_extra(mut self, extra: Vec<u8>) -> Self { self.local_extra = extra; self }
    pub(crate) fn with_comment(mut self, comment: &[u8]) -> Self { self.comment = comment.to_vec(); self }
    pub(crate) fn local_name(&self) -> &'a str { self.local_name.unwrap_or(self.name) }
}
pub(crate) fn write_test_zip(path: &Path, entries: &[ZipTestEntry<'_>]) {
    ...
    push_zip_u16(&mut file_bytes, 0); push_zip_u16(&mut file_bytes, 0);
    push_zip_u16(&mut file_bytes, entries.len() as u16); push_zip_u16(&mut file_bytes, entries.len() as u16);
    push_zip_u32(&mut file_bytes, central_directory_size); push_zip_u32(&mut file_bytes, central_directory_offset);
...
fn zip_test_sizes(entry: &ZipTestEntry<'_>) -> (usize, usize, u32) {
    let compressed = entry.data.len();
    let uncompressed = if entry.method == 8 {
        deflate_decompress_bytes(entry.data).map(|d| d.len()).unwrap_or(compressed)
    } else {
        compressed
    };
    let crc_src = if entry.method == 8 {
        deflate_decompress_bytes(entry.data).unwrap_or_else(|_| entry.data.to_vec())
    } else {
        entry.data.to_vec()
    };
    (compressed, uncompressed, zip_crc32(&crc_src))
}
fn zip_header_size_fields(entry: &ZipTestEntry<'_>, compressed: usize, uncompressed: usize) -> (u32, u32) {
    if entry.data_descriptor {
        ...
        (
            zip_test_u32(compressed, "compressed size"),
            zip_test_u32(uncompressed, "uncompressed size"),
        )
    }
}
pub(crate) fn write_zip_local_header(out: &mut Vec<u8>, entry: &ZipTestEntry<'_>) {
    let local_extra = zip_local_extra(entry);
    let (compressed, uncompressed, crc) = zip_test_sizes(entry);
    let (csize, usize_) = zip_header_size_fields(entry, compressed, uncompressed);
    ...
    push_zip_u16(out, 0);
    push_zip_u16(out, 0);
    push_zip_u32(out, crc);
    push_zip_u32(out, csize);
    push_zip_u32(out, usize_);
    ...
    if entry.data_descriptor {
        ...
        if entry.force_zip64 {
            push_zip_u64(out, compressed as u64);
            push_zip_u64(out, uncompressed as u64);
        } else {
            push_zip_u32(out, zip_test_u32(compressed, "compressed size"));
            push_zip_u32(out, zip_test_u32(uncompressed, "uncompressed size"));
        }
    }
}
pub(crate) fn write_zip_central_header(out: &mut Vec<u8>, entry: &ZipTestEntry<'_>, local_header_offset: u32) {
    ...
    push_zip_u32(out, if entry.force_zip64 { u32::MAX } else { local_header_offset });
...
fn zip_with_zip64(extra: &[u8], fields: &[u64]) -> Vec<u8> {
    let mut out = extra.to_vec();
    out.extend_from_slice(&zip64_extended_info_extra_bytes(fields));
    out
}
pub(crate) fn zip_local_extra(entry: &ZipTestEntry<'_>) -> Vec<u8> {
    if entry.force_zip64 {
        let (c, u, _) = zip_test_sizes(entry);
        zip_with_zip64(&entry.local_extra, &[u as u64, c as u64])
    } else {
        entry.local_extra.clone()
    }
}
pub(crate) fn zip_central_extra(entry: &ZipTestEntry<'_>, local_header_offset: u32) -> Vec<u8> {
    if entry.force_zip64 {
        let (c, u, _) = zip_test_sizes(entry);
        zip_with_zip64(&entry.central_extra, &[u as u64, c as u64, local_header_offset as u64])
    } else {
        entry.central_extra.clone()
    }
}
pub(crate) fn zip64_extended_info_extra_bytes(fields: &[u64]) -> Vec<u8> {
    let mut payload = Vec::new();
    for field in fields { push_zip_u64(&mut payload, *field); }
...
pub(crate) fn push_zip_u16(out: &mut Vec<u8>, value: u16) { out.extend_from_slice(&value.to_le_bytes()); }
pub(crate) fn push_zip_u32(out: &mut Vec<u8>, value: u32) { out.extend_from_slice(&value.to_le_bytes()); }
pub(crate) fn push_zip_u64(out: &mut Vec<u8>, value: u64) { out.extend_from_slice(&value.to_le_bytes()); }
pub(crate) fn set_zip_u16_at(bytes: &mut [u8], offset: usize, value: u16) { bytes[offset..offset + 2].copy_from_slice(&value.to_le_bytes()); }
pub(crate) fn set_zip_u32_at(bytes: &mut [u8], offset: usize, value: u32) { bytes[offset..offset + 4].copy_from_slice(&value.to_le_bytes()); }
pub(crate) fn set_zip_u64_at(bytes: &mut [u8], offset: usize, value: u64) { bytes[offset..offset + 8].copy_from_slice(&value.to_le_bytes()); }
...
pub(crate) fn zip_unicode_path_extra_bytes(header_name: &[u8], unicode_name: &str) -> Vec<u8> {
    ...
    zip_extra_field_bytes(0x7075, &payload)
}
