...
define_subcommands! {
    @ Commands { Read(ReadArgs) => command(about = "Read bounded file content with exact recovery refs"), Find(FindArgs) => command( about = "Search local text and return compact matches", alias = "search" ), Grep(FindArgs) => command(about = "Grep-style search: regex under the ripgrep backend, literal otherwise (use find for literal-only)"), Glob(GlobArgs) => command(about = "List matching paths without dumping file contents"), Tree(TreeArgs) => command(about = "Inspect a bounded directory tree"), Edit(EditArgs) => command(about = "Apply multi-hunk find/replace edits to one file with undo refs"), Recall(RecallArgs) => command(about = "Search payloads already stored in the recovery cache"), Fetch(FetchArgs) => command(about = "Fetch an http(s) URL via curl with a TTL cache and exact refs"), Run(RunArgs) => command( alias = "shell", alias = "rn", about = "Run a command with status-truth telemetry" ), Ingest(IngestArgs) => command(about = "Ingest text or a file into a compact TokenZero capsule"), Expand(ExpandArgs) => command(about = "Recover exact bytes from one or more prior TokenZero refs"), SessionOpen(CommonArgs) => command(name = "session-open", about = "Open a bounded manifest+delta session"), Mem(CommonArgs) => command(about = "Inspect recovery-cache state"), Rewrite(RewriteArgs) => command(about = "Rewrite a shell command with TokenZero-safe routing") => command(alias = "rewrite-command"), Hook(HookArgs) => command(about = "Agent-harness hook adapters: stdin JSON in, decision JSON out"), Discover(CommonArgs) => command(about = "List local TokenZero tool-discovery metadata"), Doctor(DoctorArgs) => command(about = "Check local TokenZero health and next steps"), Stats(CommonArgs) => command(about = "Print local TokenZero usage statistics"), Pulse(PulseArgs) => command(about = "Inspect or sync local Pulse telemetry"), SessionLedger(SessionLedgerArgs) => command( about = "Session cost ledger: token-turns (mass × turns_remaining); headline DPMT", alias = "ledger" ), Cache(CacheArgs) => command(about = "Inspect or prune TokenZero recovery-cache state"), Install(InstallArgs) => command(about = "Plan or apply local integration writes with rollback data"), Init(InitArgs) => command(about = "Compatibility alias for install --mcp --agent <name>"), Clients(ClientsArgs) => command( about = "Inspect AI client TokenZero integration state", alias = "client" ), ClientStatus(ClientStatusArgs) => command(name = "client-status", about = "Alias for clients detect"), Capabilities(CapabilitiesArgs) => command( about = "Print the machine-readable CLI contract for agents", alias = "capability", alias = "capabilites" ), RobotDocs(RobotDocsArgs) => command( name = "robot-docs", about = "Print in-tool documentation for agents", alias = "robot-doc", alias = "robotdocs" ), CachePack(CachePackArgs) => command(name = "cache-pack", about = "Build a daemonless prompt-cache pack with stable prefix and volatile refs"), Bench(BenchArgs) => command(about = "Run benchmark suites (eval artifact; not agent-primary)"), McpServer(McpServerArgs) => command(name = "mcp-server", about = "Run the TokenZero MCP server (stdio; per-op tools or codemode surface)"), McpSmoke(ArtifactArgs) => command(name = "mcp-smoke", about = "Smoke-test the MCP server (eval artifact)"), McpSoak(ArtifactArgs) => command(name = "mcp-soak", about = "Soak-test the MCP server (eval artifact)"), ExactRecoveryShell(ExactRecoveryShellArgs) => command(name = "exact-recovery-shell", about = "Audit exact byte recovery over shell corpora (eval artifact)"), ExactRecoveryAudit(ExactRecoveryAuditArgs) => command(name = "exact-recovery-audit", about = "Audit exact byte recovery guarantees (eval artifact)"), CodeMode(CodeModeArgs) => command( name = "codemode", about = "Compose multi-step plans on the same base tools as MCP (fewer round-trips, Cloudflare-style)", long_about = "Execute JS-like plans that compose the same TokenZero operations as MCP (zero.read, zero.find, zero.shell, ...) in one call for faster multi-step workflows.\n\nTier B shell trampoline (universal execution): one shell line runs a full plan. Pass the plan inline, via --plan-file, via --stdin / PLAN=-, or pipe a heredoc when stdin is not a TTY. Emits the same tokenzero.codemode.v1 envelope and tz:// refs as the MCP path. Cap harness-visible output with --budget / --max-visible-tokens. Failures are typed (status=error + error.kind); no silent partials.\n\nExample:\n  tokenzero codemode --json --budget 2000 --root . --stdin <<'EOF'\n  const f = await zero.read(\"README.md\");\n  return { ref: f.ref };\n  EOF\n\nDiscovery: tokenzero codemode 'search:read' | tokenzero codemode 'describe:zero.read'\n\nCache: defaults to the same recovery-cache.json as CLI expand/MCP so refs mintable by codemode expand on the next call. Override with --cache-path / TOKENZERO_CACHE_PATH when using an isolated store (wrong root yields store_mismatch naming both paths)." ), HarmEval(ArtifactArgs) => command(name = "harm-eval", about = "Run the harm evaluation harness (eval artifact)"), ProtectedAnchorAudit(ProtectedAnchorAuditArgs) => command(name = "protected-anchor-audit", about = "Audit protected-anchor preservation (eval artifact)"), FalseSuccessShell(FalseSuccessShellArgs) => command(name = "false-success-shell", about = "Audit false-success detection on shell output (eval artifact)"), RepoInventory(ArtifactArgs) => command(name = "repo-inventory", about = "Inventory repository surfaces for audits (eval artifact)"), PromptCachePack(ArtifactArgs) => command(name = "prompt-cache-pack", about = "Build a prompt-cache pack artifact (eval)"), InstallSmoke(InstallSmokeArgs) => command(name = "install-smoke", about = "Plan an install probe in a disposable temporary root; --apply runs apply plus rollback there"), PackageAudit(PackageAuditArgs) => command(name = "package-audit", about = "Audit release packaging (eval artifact)"), ShellMatrix(ShellMatrixArgs) => command(name = "shell-matrix", about = "Audit shell compatibility matrix (eval artifact)"), OsReachAudit(OsReachAuditArgs) => command(name = "os-reach-audit", about = "Audit OS reach/portability claims (eval artifact)"), OsReleaseArtifact(OsReleaseArtifactArgs) => command(name = "os-release-artifact", about = "Produce OS release artifacts (eval)"), OneShotEval(OneShotEvalArgs) => command(name = "one-shot-eval", about = "Run one-shot evaluation harness (eval artifact)"), SourceCurrencyAudit(SourceCurrencyAuditArgs) => command(name = "source-currency-audit", about = "Audit source freshness/currency (eval artifact)"), AdapterApprovalAudit(AdapterApprovalAuditArgs) => command(name = "adapter-approval-audit", about = "Audit adapter approval state (eval artifact)"), AdapterApprovalTemplate(AdapterApprovalTemplateArgs) => command(name = "adapter-approval-template", about = "Emit adapter approval template (eval artifact)"), ClaimAudit(ClaimAuditArgs) => command(name = "claim-audit", about = "Audit marketing/claim evidence against artifacts (eval artifact)"), CompletionAudit(CompletionAuditArgs) => command(name = "completion-audit", about = "Audit task completion claims (eval artifact)"), SecurityPrivacyAudit(SecurityPrivacyAuditArgs) => command(name = "security-privacy-audit", about = "Audit security/privacy posture (eval artifact)"), ArtifactHandoff(ArtifactHandoffArgs) => command(name = "artifact-handoff", about = "Package artifacts for handoff (eval)"), Reach(ReachArgs) => command(about = "Audit repository reach/coverage (eval artifact)"), WsSkeleton(WsSkeletonArgs) => command(name = "ws-skeleton", about = "Emit workspace skeleton scaffold (eval)"), Quote(QuoteArgs) => command(about = "Quote shell arguments safely for the current platform"), }
...
define_args! {
    ...
    @ CodeModeArgs(#[doc = "CodeMode plan (JS-style zero.token.compact / expand) as a positional argument. Use `-` to read the plan from stdin."] #[arg(value_name = "PLAN")] plan: Option<String>; #[doc = "CodeMode plan as an explicit flag; kept for router compatibility."] #[arg(short = 'p', long = "plan", value_name = "PLAN")] plan_flag: Option<String>; #[doc = "Read plan from a file instead of inline. Supports .txt and .js extensions."] #[arg(long = "plan-file", value_name = "PATH")] plan_file: Option<PathBuf>; #[doc = "Read plan from stdin (Tier B trampoline; heredoc-friendly)."] #[arg(long)] stdin: bool; #[doc = "Workspace root used for CodeMode file, shell, and recovery-cache boundaries."] #[arg(long)] root: Option<PathBuf>; #[doc = "Additional allowed roots for plans that must intentionally cross the workspace boundary."] #[arg(long)] allowed_root: Vec<PathBuf>; #[doc = "Override the CodeMode recovery cache path."] #[arg(long)] cache_path: Option<PathBuf>; #[doc = "Visible-token budget for each underlying TokenZero operation (alias: --budget)."] #[arg(long, default_value_t = 4000, alias = "budget", visible_alias = "budget")] max_visible_tokens: usize; #[arg(long, alias = "timeout", alias = "timout", value_name = "SECONDS")] timeout_seconds: Option<u64>; #[arg(long)] json: bool;)
}
...
impl CodeModeArgs {
    /// Resolve the plan from exactly one source: inline/`--plan`, `--plan-file`, or stdin.
    ///
    /// Stdin is selected by `--stdin`, `PLAN=-`, or a non-TTY stdin when no other source is set
    /// (heredoc / pipe trampoline). Conflicting sources return `InvalidInput`.
    pub(crate) fn plan_text(&self) -> Result<String, std::io::Error> {
        use std::io::{IsTerminal, Read};

        let inline = self.plan_flag.as_deref().or(self.plan.as_deref());
        let dash = matches!(inline, Some("-"));
        let from_file = self.plan_file.is_some();
        let from_inline = inline.is_some() && !dash;
        let explicit_stdin = self.stdin || dash;
        let auto_stdin =
            !from_file && !from_inline && !explicit_stdin && !std::io::stdin().is_terminal();

        if (from_file && (from_inline || explicit_stdin)) || (from_inline && explicit_stdin) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "conflicting plan sources: use exactly one of PLAN/--plan, --plan-file, or --stdin (PLAN=-)",
            ));
        }
        ...
        if explicit_stdin || auto_stdin {
            let mut buf = String::new();
            std::io::stdin().read_to_string(&mut buf)?;
            return Ok(buf);
        }
        Ok(inline.unwrap_or("").to_string())
    }
}
