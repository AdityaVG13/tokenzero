...
define_args! {
    ...
    @ McpServerArgs(#[doc = "Launch the explicit classic MCP compatibility surface. Aggregate CodeMode belongs to ZeroStack."] #[arg(long, default_value = "mcp", value_name = "MODE")] mode: String; #[arg(long)] allowed_root: Vec<PathBuf>; #[arg(long)] cache_path: Option<PathBuf>; #[arg(long, default_value = "auto")] default_mode: String; #[arg(long, alias = "timeout", value_name = "SECONDS")] shell_timeout_seconds: Option<u64>; #[arg(long, value_name = "SECONDS")] idle_timeout_seconds: Option<u64>; #[doc = "Run a crash-transparent supervisor that owns the client stdio pipes and automatically respawns the inner MCP server if it ever dies."] #[arg(long)] supervise: bool; #[doc = "Backward-compatible alias for --mode; only mcp is accepted locally."] #[arg(long, value_name = "SURFACE")] tool_surface: Option<String>;)
