...
fn release_os() -> &'static str {
    if cfg!(windows) { "windows" } else if cfg!(target_os = "linux") { "linux" }
    else if cfg!(target_os = "macos") { "macos" } else { std::env::consts::OS }
}

// --- Const surface definitions ---
const CORE_SURFACES: &[&str] = &["install", "doctor", "shell", "mcp", "cache_pack"];

fn core_surface_row(surface: &str, ok: bool, evidence: &str, details: serde_json::Value) -> serde_json::Value {
    json!({"surface":surface,"ok":ok,"daemon_required":false,"global_writes":false,"evidence":evidence,"details":details})
}

pub(crate) fn run_shell_matrix(output_json: PathBuf, output_md: Option<PathBuf>) -> Result<serde_json::Value> {
    let exe = std::env::current_exe()?;
    let temp = tempdir()?;
    let cache = temp.path().join("matrix-cache.json");
    let mut rows = Vec::new();
    let mut direct = Command::new(&exe);
    direct.args(["run","--cache-path"]).arg(&cache).args(["--","echo","ok"]);
    rows.push(run_matrix_row("direct", &mut direct));
    if cfg!(unix) {
        let mut env_cmd = Command::new("env");
        env_cmd.arg("-i").arg(&exe).args(["run","--cache-path"]).arg(&cache).args(["--","echo","ok"]);
        rows.push(run_matrix_row("env-i", &mut env_cmd));
        let inv = format!("{} run --cache-path {} -- echo ok", exe.display(), cache.display());
        for clean in [false, true] {
            for sh in ["/bin/sh","/bin/bash","/bin/zsh"] {
                if !Path::new(sh).exists() { continue; }
                let mut cmd = if clean { Command::new("env") } else { Command::new(sh) };
                if clean { cmd.arg("-i").arg(sh); }
                cmd.arg("-c").arg(&inv);
                rows.push(run_matrix_row(&if clean { format!("env-i {sh} -c") } else { format!("{sh} -c") }, &mut cmd));
            }
        }
    }
    if cfg!(windows) {
        let args = vec![exe.display().to_string(),"run".into(),"--cache-path".into(),cache.display().to_string(),"--".into(),"echo".into(),"ok".into()];
        let mut cmd = Command::new("cmd"); cmd.arg("/C").arg(quote_for("cmd",&args));
        rows.push(run_matrix_row("cmd /C", &mut cmd));
        let mut ps = Command::new("powershell"); ps.arg("-NoProfile").arg("-Command").arg(format!("& {}", quote_for("powershell",&args)));
    ...
    finish_artifact(&output_json, output_md.as_deref(), report, "Rust shell matrix")
}

pub(crate) fn run_os_reach_audit(output_json: PathBuf, output_md: Option<PathBuf>, root: PathBuf, os_artifacts: Vec<PathBuf>, release_approval: bool) -> Result<serde_json::Value> {
    let temp = tempdir()?;
    ...
    let rcid_list: Vec<_> = os_rows.iter().filter(|r| r["claim_ready"]==true).filter_map(|r| r["release_candidate_id"].as_str()).filter(|id| !id.trim().is_empty()).map(str::to_string).collect();
    let unique_ids: Vec<_> = rcid_list.iter().fold(Vec::new(), |mut ids, id| { if !ids.contains(id) { ids.push(id.clone()); } ids });
    let same_rc = all_run && rcid_list.len()==release_oses.len() && unique_ids.len()==1;

    let mut blocked: Vec<_> = os_rows.iter().filter(|r| r["claim_ready"]!=true).map(|r| format!("{} not run with shell and install artifacts", r["os"].as_str().unwrap_or("unknown"))).collect();
    if all_run && !same_rc { blocked.push("OS release artifacts are not from the same release candidate".into()); }
    if blocked.is_empty() && !release_approval { blocked.push("explicit release approval not granted".into()); }
    let claim_ok = all_run && same_rc && release_approval;
    let ok = sm["ok"]==true && install["ok"]==true && install["global_writes"]==false && reach["daemon_required"]==false && reach["global_writes"]==false;
...
pub(crate) fn run_os_release_artifact(output_json: PathBuf, output_md: Option<PathBuf>, root: PathBuf) -> Result<serde_json::Value> {
    let temp = tempdir()?;
    let sm = run_shell_matrix(temp.path().join("shell-matrix.json"), None)?;
    let install = run_install_smoke(None)?;
    let reach = run_reach(root, None)?;
    ...
    let gw = reach["global_writes"]!=false || install["global_writes"]!=false || cs.iter().any(|r| r["global_writes"]!=false);
    ...
    finish_artifact(&output_json, output_md.as_deref(), report, "OS release artifact")
}

pub(crate) fn run_core_surface_audit(shell_matrix: &serde_json::Value, install_smoke: &serde_json::Value) -> Result<Vec<serde_json::Value>> {
    let temp = tempdir()?;
    let root = temp.path().to_path_buf();
    let doctor = doctor_report(&DoctorArgs { root: Some(root.clone()), cache_path: Some(root.join("recovery-cache.json")), runtime: true, json: true, robot_triage: false, fix: false, dry_run: false, explain: None, command: None });
    let cache_engine = TokenZeroEngine::new(EngineConfig { allowed_roots: default_allowed_roots(&root), cache_path: root.join("recovery-cache.json"), max_visible_tokens: 4000, mode: Mode::Structured, shell_timeout: default_shell_timeout(), mcp_idle_timeout: None, ..EngineConfig::for_root(&root) });
    let cp = cache_engine.cache_pack("agent");
    let mcp = run_mcp_artifact(root.join("mcp-smoke.json"), None, 1)?;
    Ok(vec![
        core_surface_row("install", install_smoke["ok"]==true && install_smoke["global_writes"]==false, "install-smoke disposable local root", json!({"schema_version":install_smoke["schema_version"],"global_writes":install_smoke["global_writes"]})),
        core_surface_row("doctor", doctor["ok"]==true, "doctor --runtime on disposable local root", json!({"schema_version":doctor["schema_version"],"root":doctor["root"]})),
        core_surface_row("shell", shell_matrix["ok"]==true, "shell-matrix current host", json!({"schema_version":shell_matrix["schema_version"],"windows":shell_matrix["windows"],"linux":shell_matrix["linux"],"macos":shell_matrix["macos"]})),
        core_surface_row("mcp", mcp["ok"]==true && mcp["unexpected_exits"]==0, "mcp-smoke local stdio process", json!({"schema_version":mcp["schema_version"],"unexpected_exits":mcp["unexpected_exits"]})),
        core_surface_row("cache_pack", cp.status=="ok"||cp.status=="degraded", "cache-pack local recovery cache", json!({"tool":cp.tool,"status":cp.status,"refs":cp.refs.len()})),
    ])
}

pub(crate) fn load_os_release_artifacts(paths: &[PathBuf]) -> Result<Vec<serde_json::Value>> {
    paths.iter().map(|p| {
        let mut a: serde_json::Value = serde_json::from_slice(&fs::read(p).with_context(|| format!("read {}", p.display()))?).with_context(|| format!("parse {}", p.display()))?;
