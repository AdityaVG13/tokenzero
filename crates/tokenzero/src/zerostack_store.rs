...
//! **not** pin every call root into one store by default. Default store is
//! derived from the call root. Shared/meta store requires explicit opt-in
//! (`TOKENZERO_SHARED_STORE` / `ZEROSTACK_SHARED_STORE` = 1|on|true|yes).
...
/// Pure store-root selection (wqw.2).
///
/// Algorithm:
...
/// 3. Else → `None` (caller falls back to legacy `repo_root/.tokenzero/...`).
///
...
fn resolve_default_cache_path(
    ...
    resolve_default_cache_path_with_store(
        repo_root,
        unified_relative,
        legacy_relative,
        zerostack_store_or_detect(repo_root),
    )
}

fn resolve_default_cache_path_with_store(
    repo_root: &Path,
    unified_relative: &str,
    legacy_relative: &str,
...
/// Whether `store` is under `root` (same project). Used by doctor mismatch.
...
/// Pure resolution report for tests and doctor (no process env except when
/// callers pass live env into the `*_from_process` wrapper).
...
mod tests {
    ...
    // ── wqw.2 multi-project isolation ─────────────────────────────────────

    #[test]
    fn global_store_pin_ignored_without_shared_opt_in() {
        let a = tempdir().unwrap();
        let b = tempdir().unwrap();
        let shared = tempdir().unwrap();
        let pin = shared.path().as_os_str();

        let store_a = resolve_store_root_with_env(a.path(), Some(pin), false);
        let store_b = resolve_store_root_with_env(b.path(), Some(pin), false);
        assert!(store_a.is_none(), "no opt-in → ignore global pin");
        assert!(store_b.is_none());

        let cache_a = resolve_default_cache_path_with_store(
            a.path(),
            "tokenzero/recovery-cache.json",
            ".tokenzero/recovery-cache.json",
            store_a,
        );
        let cache_b = resolve_default_cache_path_with_store(
            b.path(),
            "tokenzero/recovery-cache.json",
            ".tokenzero/recovery-cache.json",
            store_b,
        );
        assert_eq!(cache_a, a.path().join(".tokenzero/recovery-cache.json"));
        assert_eq!(cache_b, b.path().join(".tokenzero/recovery-cache.json"));
        assert_ne!(cache_a, cache_b);
    }

    #[test]
    fn two_roots_isolate_default_stores_despite_global_pin() {
        let a = tempdir().unwrap();
        let b = tempdir().unwrap();
        let shared = tempdir().unwrap();
        // Simulate former bug: pin present, no opt-in → still isolated.
        let pin = Some(shared.path().as_os_str());
        let ca = resolve_default_cache_path_with_store(
            a.path(),
            "tokenzero/recovery-cache.json",
            ".tokenzero/recovery-cache.json",
            resolve_store_root_with_env(a.path(), pin, false),
        );
        let cb = resolve_default_cache_path_with_store(
            b.path(),
            "tokenzero/recovery-cache.json",
            ".tokenzero/recovery-cache.json",
            resolve_store_root_with_env(b.path(), pin, false),
        );
        assert!(ca.starts_with(a.path()), "{ca:?}");
        assert!(cb.starts_with(b.path()), "{cb:?}");
        assert_ne!(ca, cb);
        // Neither lands under the global pin.
        assert!(!ca.starts_with(shared.path()));
        assert!(!cb.starts_with(shared.path()));
    }

    #[test]
    fn shared_opt_in_uses_global_pin_for_both_roots() {
        let a = tempdir().unwrap();
        let b = tempdir().unwrap();
        let shared = tempdir().unwrap();
        let pin = Some(shared.path().as_os_str());
        let store_a = resolve_store_root_with_env(a.path(), pin, true).unwrap();
        let store_b = resolve_store_root_with_env(b.path(), pin, true).unwrap();
        assert_eq!(store_a, shared.path());
        assert_eq!(store_b, shared.path());
        let ca = resolve_default_cache_path_with_store(
            a.path(),
            "tokenzero/recovery-cache.json",
            ".tokenzero/recovery-cache.json",
        ...
        assert_eq!(ca, cb);
        assert_eq!(ca, shared.path().join("tokenzero/recovery-cache.json"));
    }

    #[test]
    fn local_dot_zerostack_wins_over_global_pin_even_with_opt_in() {
        let proj = tempdir().unwrap();
        let shared = tempdir().unwrap();
        fs::create_dir_all(proj.path().join(".zerostack/tokenzero")).unwrap();
        let store = resolve_store_root_with_env(proj.path(), Some(shared.path().as_os_str()), true)
            .unwrap();
        assert_eq!(store, proj.path().join(".zerostack"));
        assert!(store_is_under_project_root(&store, proj.path()));
    }

    #[test]
    fn doctor_report_flags_shared_store_mismatch() {
        let proj = tempdir().unwrap();
        let shared = tempdir().unwrap();
        ...
        assert!(report.store_project_mismatch, "{report:?}");
    ...
    #[test]
    fn doctor_report_notes_ignored_global_pin_without_opt_in() {
        let proj = tempdir().unwrap();
        let shared = tempdir().unwrap();
        let report = store_resolution_report_with_env(
            proj.path(),
        ...
        assert!(
            report
                ...
                .as_deref()
                .unwrap_or("")
                .contains("ignored for isolation"),
            "{:?}",
            report.mismatch_summary
        );
        assert!(report.effective_cache_path.starts_with(proj.path()));
    }
}
