use tokenzero_core::{render_shell, Mode, ShellRender, ShellRenderInput};
...
fn masked_zero(command: &str, stdout: &str, stderr: &str, segment: &str) -> ShellRender {
    let rendered = render_shell(ShellRenderInput {
        command,
        stdout,
        stderr,
    ...
    assert!(
        ...
        "{command}: exit 0 must not be command_failed: {rendered:?}"
    );
    assert_eq!(
    ...
    masking(&rendered, command, segment);
    rendered
}
macro_rules! masked { ($($command:expr => ($stdout:expr, $stderr:expr, $code:expr, $segment:expr);)*) => { $(if $code == 0 { masked_zero($command, $stdout, $stderr, $segment); } else { let rendered = failure($command, $stdout, $stderr, $code); masking(&rendered, $command, $segment); })* }; }
...
handled_command_lookup_does_not_poison_later_help_output {
    ...
    masked_zero(command, "", "command: --definitely-invalid: invalid option\n", "command --definitely-invalid herdr");

    let command = "command -v -x herdr || true; herdr --help 2>&1 | sed -n '1,180p'";
    masked_zero(command, "command: invalid option -- 'x'\nUsage: command [-pVv] command [arg ...]\n", "", "command -v -x herdr");
}
